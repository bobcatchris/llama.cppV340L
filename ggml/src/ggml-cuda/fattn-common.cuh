...
void launch_fattn(
    ...
    const int fill_parallel_blocks = 0
) {
    constexpr int ncols = ncols1 * ncols2;
    ...
    if (stream_k) {
        ...
        // W43 attn-concurrency arm: optional KV-split (parallel_blocks)
        // override for the latency-bound tile cells. 0 keeps the efficiency
        // scan above; > 0 pins the value (clamped to ntiles_KV); < 0 asks for
        // the maximum split (one KV tile per block). Fixed value per ne11, so
        // the combine reduction order stays deterministic run to run.
        if (fill_parallel_blocks != 0) {
            parallel_blocks = fill_parallel_blocks > 0 ? fill_parallel_blocks : ntiles_KV;
            parallel_blocks = std::min(ntiles_KV, std::max(parallel_blocks, 1));
            blocks_num.y = parallel_blocks;
        }
