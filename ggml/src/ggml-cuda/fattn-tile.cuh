...
static void launch_fattn_tile_switch_ncols2(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if constexpr (DKQ == 256 && DV == 256) {
        ...
        // W43 attn-concurrency: KV-split (parallel_blocks) override for the
        // direct decode arm, measured only (latency-bound tile, W17). Kernel
        // body and store shapes untouched; default off = the scan as shipped.
        static const char * q40_fill_env = getenv("GGML_CUDA_FATTN_TILE_Q40_FILL");
        if (q40_direct && use_gqa_opt && Q->ne[1] <= 4 && K->type == GGML_TYPE_Q4_0 && V->type == GGML_TYPE_Q4_0) {
            ...
            const int q40_fill  = q40_fill_env ? atoi(q40_fill_env) : 0;
            GGML_LOG_WARN("ggml-cuda: GGML_CUDA_FATTN_TILE_Q40_DIRECT=1, fattn tile ncols1=4 ncols2=2 q4_0 KV in-kernel (gqa_ratio %d, T %d, fill %d)\n", gqa_ratio, (int) Q->ne[1], q40_fill);
            ...
            launch_fattn<DV, 4, 2>(ctx, dst, flash_attn_tile<DKQ, DV, 4, 2, use_logit_softcap, 11>,
                nwarps, nbytes_shared, nbatch_fa, false, false, false, warp_size, q40_fill);
            return;
        }
    }

    // Q4_0-direct prefill arm (GGML_CUDA_FATTN_TILE_Q40_PREFILL): same
    // in-kernel q4_0 KV dequant as the decode arm, at the prefill instance
    // <256,256,8,2> (cols_per_block=16), so launch geometry and the
    // parallel-block scan match the f16-pool prefill arm exactly.
    if constexpr (DKQ == 256 && DV == 256) {
        static const bool q40_prefill = getenv("GGML_CUDA_FATTN_TILE_Q40_PREFILL") != nullptr;
        if (q40_prefill && use_gqa_opt && Q->ne[1] > 4 && K->type == GGML_TYPE_Q4_0 && V->type == GGML_TYPE_Q4_0) {
            const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
            const int warp_size = 32;
            constexpr size_t nbytes_shared = 0;
            const int nwarps    = ggml_cuda_fattn_tile_get_nthreads (DKQ, DV, 16, cc) / warp_size;
            const int nbatch_fa = ggml_cuda_fattn_tile_get_nbatch_fa(DKQ, DV, 16, cc);
            GGML_LOG_WARN("ggml-cuda: GGML_CUDA_FATTN_TILE_Q40_PREFILL=1, fattn tile ncols1=8 ncols2=2 q4_0 KV in-kernel (gqa_ratio %d, T %d)\n", gqa_ratio, (int) Q->ne[1]);
            // var 11 (scalar half2 stores) - the only W17-exact store shape;
            // this is a new instantiation, exactness re-proven by oracle.
            launch_fattn<DV, 8, 2>(ctx, dst, flash_attn_tile<DKQ, DV, 8, 2, use_logit_softcap, 11>,
