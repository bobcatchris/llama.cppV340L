...
template <ggml_type type, int ncols_dst, bool has_fusion, bool small_k = false, bool ALN = false, bool S2R = false, int min_ctas_per_cu = 1, bool RPB1 = false>
...
static __global__ void mul_mat_vec_q(
    ...
    // RPB1 arm (W42): one-row blocks on the share body for ncols_dst 2-4.
    // Halves the per-CTA decode state (dec[1][..] vs dec[2][..]) so the
    // T=2-4 share arms fit 8 CTAs/CU without the LB4 spill; the grid from
    // calc_launch_params must use the same value.
    constexpr int rows_per_cuda_block = (RPB1 && ncols_dst >= 2 && ncols_dst <= 4)
        ? 1 : calc_rows_per_block(ncols_dst, table_id, small_k, nwarps);
...
static std::pair<dim3, dim3> calc_launch_params(
        ...
        const bool rpb1 = false) {
    const int nwarps = calc_nwarps(type, ncols_dst, table_id);
    const int rpb = (rpb1 && ncols_dst >= 2 && ncols_dst <= 4)
        ? 1 : calc_rows_per_block(ncols_dst, table_id, small_k, nwarps);
...
// RPB1 arm (W42 desk): rows_per_cuda_block = 1 for the T = 2..4 share
// launches. W23 M2 names register-capped occupancy as the co-limiter
// (share arms hold dec[2][..]; 77/84/112 VGPRs -> 6/6/4 CTAs/CU vs 8 at
// <= 64); halving the row state is the diet that reaches 8 CTAs/CU without
// the LB4 spill. The per-row arithmetic is unchanged (the row loop runs
// once per CTA instead of twice with the same decode/apply sequence), so
// the arm is bit-exact by construction - still oracled per the W30 law.
// Mutually exclusive with s2r (no row pair to share y across) and aln;
// composes with the LB gate for the launch_bounds hint.
static bool mmvq_rpb1_env_enabled() {
    static const bool on = [] {
        const char * env = getenv("GGML_CUDA_MMVQ_RPB1");
        if (env != nullptr && env[0] == '1') {
            GGML_LOG_WARN("ggml-cuda: GGML_CUDA_MMVQ_RPB1=1, one-row blocks for the T=2-4 share arms (register diet)\n");
            return true;
        }
        return false;
    }();
...
// shared by the grid calc (switch_ncols_dst) and the launch gate
// (switch_fusion) so the grid always matches the kernel's
// rows_per_cuda_block
static bool mmvq_rpb1_active(ggml_type type, int ncols_dst, bool s2r) {
    return ncols_dst >= 2 && ncols_dst <= 4 &&
        mmvq_rpb1_env_enabled() &&
        mmvq_share_env_enabled(type) &&
        !(s2r && mmvq_s2r_env_enabled(type));
}
...
static void mul_mat_vec_q_switch_fusion(
        ...
        const uint32_t ids_stride, const bool aln, const bool s2r, cudaStream_t stream, const bool rpb1 = false) {
    ...
    const bool share = (mmvq_share_env_enabled(type) || s2r_on)
        && c_ncols_dst >= 2 && c_ncols_dst <= 4;
    // rpb1 arrives fully evaluated from switch_ncols_dst (grid must agree);
    // the ncols guard is compile-time here
    const bool rpb1_on = rpb1 && c_ncols_dst >= 2 && c_ncols_dst <= 4;
    bool aln_on = false;
    if constexpr (c_ncols_dst == 4) {
        aln_on = share && aln && mmvq_type_has_aln(type) && !s2r_on && !rpb1_on;
    }
    ...
    const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
    auto launch_variant = [&](auto aln_c, auto s2r_c, auto lb_c, auto rpb1_c) {
        constexpr bool kAln = decltype(aln_c)::value;
        constexpr bool kS2r = decltype(s2r_c)::value;
        constexpr int  kLb  = decltype(lb_c)::value;
        constexpr bool kRpb1 = decltype(rpb1_c)::value;
        ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, kAln, kS2r, kLb, kRpb1>, launch_params,
            vx, vy, vy_aln, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
            channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
            sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, share);
    };
    if (rpb1_on) {
        // W42 rpb1 arm: one-row share blocks; the LB gate rides along as the
        // launch_bounds hint (4 forces the 64-VGPR budget the diet aims for)
        if (mmvq_lb_env_value() == 4) {
            launch_variant(std::false_type{}, std::false_type{}, std::integral_constant<int, 4>{}, std::true_type{});
            return;
        }
        launch_variant(std::false_type{}, std::false_type{}, std::integral_constant<int, 1>{}, std::true_type{});
        return;
    }
    if (aln_on) {
        if (s2r_on) { launch_variant(std::true_type{},  std::true_type{},  std::integral_constant<int, 1>{}, std::false_type{}); }
        else         { launch_variant(std::true_type{},  std::false_type{}, std::integral_constant<int, 1>{}, std::false_type{}); }
    } else {
        if (s2r_on) {
            launch_variant(std::false_type{}, std::true_type{},  std::integral_constant<int, 1>{}, std::false_type{});
        } else {
            if constexpr (c_ncols_dst >= 2 && c_ncols_dst <= 4) {
                const int lb = mmvq_lb_env_value();
                if (lb == 2) {
                    launch_variant(std::false_type{}, std::false_type{}, std::integral_constant<int, 2>{}, std::false_type{});
                    return;
                }
                if (lb == 4) {
                    launch_variant(std::false_type{}, std::false_type{}, std::integral_constant<int, 4>{}, std::false_type{});
                    return;
                }
            }
            launch_variant(std::false_type{}, std::false_type{}, std::integral_constant<int, 1>{}, std::false_type{});
        }
    }
}
...
static void mul_mat_vec_q_switch_ncols_dst(
        ...
        const int ids_stride, const bool aln, const bool s2r, cudaStream_t stream) {
    ...
    // W42 rpb1 arm: decide once so the grid and the kernel-side
    // rows_per_cuda_block agree (switch_fusion launches the same predicate)
    const bool rpb1 = mmvq_rpb1_active(type, ncols_dst, s2r);
    ...
    switch (ncols_dst) {
        ...
        case 2: {
            ...
            std::pair<dim3, dim3> dims = calc_launch_params<type>(c_ncols_dst, nrows_x, nchannels_dst, nsamples_dst, warp_size, table_id, false, rpb1);
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(vx, vy, vy_aln, ids, fusion, dst, ncols_x, nchannels_y_fd, stride_row_x, stride_col_y, stride_col_dst,
                 ...
                 dims.first, dims.second, 0, ids_stride, aln, s2r, stream, rpb1);        } break;
        case 3: {
        ...
        case 5: {
            ...
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(vx, vy, vy_aln, ids, fusion, dst, ncols_x, nchannels_y_fd, stride_row_x, stride_col_y, stride_col_dst,
                 channel_ratio_fd, stride_channel_x, stride_channel_y, stride_channel_dst,
                 sample_ratio_fd, stride_sample_x, stride_sample_y, stride_sample_dst,
                 dims.first, dims.second, 0, ids_stride, aln, s2r, stream);        } break;
        case 6: {
        ...
        case GGML_TYPE_Q1_0:
            mul_mat_vec_q_switch_ncols_dst<GGML_TYPE_Q1_0>
                (vx, vy, vy_aln, ids, fusion, dst, ncols_x, nrows_x, ncols_dst, stride_row_x, stride_col_y, stride_col_dst,
                 ...
                 nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, aln, s2r, stream);
            break;
        case GGML_TYPE_Q4_0:
...
void ggml_cuda_mul_mat_vec_q(
    ...
    mul_mat_vec_q_switch_type(
        ...
        ne03,              ne3,           s03, s13,              s3,               ids_stride, mmvq_aln, mmvq_s2r, stream);
}
