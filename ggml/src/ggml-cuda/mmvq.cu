...
template <ggml_type type, int ncols_dst, bool has_fusion, bool small_k = false, bool ALN = false, bool S2R = false, bool WS2R = false>
...
#pragma unroll
                ...
                if constexpr (S2R && WS2R) {
                    // wide-s2r: all token y-preloads in flight before any
                    // apply; same loads, same apply order, bit-exact
                    mmvq_yw8 w[ncols_dst];
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
                        w[j] = vec_dot_iq3_s_q8_1_preload(&y[j*stride_col_y + kby], kqs);
                    }
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
#pragma unroll
                        for (int i = 0; i < rows_per_cuda_block; ++i) {
                            tmp[j][i] += vec_dot_iq3_s_q8_1_apply_u(w[j], dec[i], scale_share[i], d_share[i]);
                        }
                    }
                } else {
#pragma unroll
...
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
                        w[j] = vec_dot_iq3_xxs_q8_1_preload(&y[j*stride_col_y + kby], kqs);
                    }
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
#pragma unroll
                        for (int i = 0; i < rows_per_cuda_block; ++i) {
                            tmp[j][i] += vec_dot_iq3_xxs_q8_1_apply_u(w[j], dec[i], ls_share[i], d_share[i]);
                        }
                    }
                } else {
#pragma unroll
...
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
                        w[j] = vec_dot_iq4_xs_q8_1_preload(&y[j*stride_col_y + kby], kqs);
                    }
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
#pragma unroll
                        for (int i = 0; i < rows_per_cuda_block; ++i) {
                            tmp[j][i] += vec_dot_iq4_xs_q8_1_apply_u(w[j], dec[i], scale_share[i], d_share[i]);
                        }
                    }
                } else {
#pragma unroll
...
#pragma unroll
                ...
                if constexpr (S2R && WS2R) {
                    // wide-s2r: all token y-preloads in flight before any
                    // apply; same loads, same apply order, bit-exact
                    mmvq_ywq2 w[ncols_dst];
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
                        w[j] = vec_dot_q4_K_q8_1_preload(&y[j*stride_col_y + kby], kqs);
                    }
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
#pragma unroll
                        for (int i = 0; i < rows_per_cuda_block; ++i) {
                            tmp[j][i] += vec_dot_q4_K_q8_1_apply_u(w[j], dec[i], sc_share[i], m_share[i], dm_share[i]);
                        }
                    }
                } else {
#pragma unroll
...
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
                        w[j] = vec_dot_q5_K_q8_1_preload(&y[j*stride_col_y + kby], kqs);
                    }
#pragma unroll
                    for (int j = 0; j < ncols_dst; ++j) {
#pragma unroll
                        for (int i = 0; i < rows_per_cuda_block; ++i) {
                            tmp[j][i] += vec_dot_q5_K_q8_1_apply_u(w[j], dec[i], sc_share[i], m_share[i], dm_share[i]);
                        }
                    }
                } else {
#pragma unroll
...
// wide-s2r (C3, W25 desk): with a per-type s2r arm engaged, hoist all
// ncols_dst y-preloads above the apply loops (4 x 36 B in flight per
// lane-iteration at the served T=4 instead of one). Bit-exact by
// construction: identical loads and apply order, only load timing moves.
// One global gate, inert unless a per-type *_S2R gate is also set; read
// behind the s2r engagement short-circuit, so this line only prints when
// the s2r wiring actually reaches it (E-117 witness property). Printed at
// WARN: the served log drops ggml INFO.
static bool mmvq_ws2r_env_enabled() {
    static const bool on = [] {
        const char * env = getenv("GGML_CUDA_MMVQ_WIDE_S2R");
        if (env != nullptr && env[0] == '1') {
            GGML_LOG_WARN("ggml-cuda: GGML_CUDA_MMVQ_WIDE_S2R=1, wide-s2r y-preload hoist enabled for engaged s2r arms (ncols_dst=4)\n");
            return true;
        }
        return false;
    }();
...
static void mul_mat_vec_q_switch_fusion(
        ...
        const uint32_t ids_stride, const bool aln, const bool s2r, cudaStream_t stream) {
    ...
    if constexpr (c_ncols_dst == 4) {
        ws2r_on = s2r_on && mmvq_ws2r_env_enabled();
    }
    bool aln_on = false;
    if constexpr (c_ncols_dst == 4) {
    ...
    const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
    auto launch_variant = [&](auto aln_c, auto s2r_c, auto ws2r_c) {
        constexpr bool kAln = decltype(aln_c)::value;
        constexpr bool kS2r = decltype(s2r_c)::value;
        constexpr bool kW2r = decltype(ws2r_c)::value;
        ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, kAln, kS2r, kW2r>, launch_params,
            vx, vy, vy_aln, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
            channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
            sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, share);
    };
    if (aln_on) {
        ...
        if (s2r_on) {
            if (ws2r_on) { launch_variant(std::false_type{}, std::true_type{},  std::true_type{});  }
            else         { launch_variant(std::false_type{}, std::true_type{},  std::false_type{}); }
        } else {
