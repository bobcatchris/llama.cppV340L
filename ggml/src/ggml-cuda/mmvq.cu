...
void ggml_cuda_mul_mat_vec_q(
    ...
    bool   src1_q8_1_fresh = false;
    ggml_cuda_pool_alloc<char> src1_q8_1(ctx.pool());
    if (ids == nullptr && ggml_cuda_q81_act_cache::cacheable(src1)) {
        const ggml_cuda_q81_act_cache::key q81_key =
            ggml_cuda_q81_act_cache::make_key(src1, ctx.device, stream, mmvq_aln ? 1 : 0);
