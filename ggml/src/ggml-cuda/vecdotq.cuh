...
// --- ldsy: staged y-operand consumers for the LDS-y share arms (W7 receipt) -
// The CTA cooperatively copies each needed q8_1 y block into a 13-dword LDS
// record: word 0 = ds, words 1..8 = qs dwords 0..7, words 9..12 pad (never
// read; the stride keeps the record reads bank-conflict-free). The apply_lds
// twins replay the identical arithmetic from the record, so results are
// bit-exact vs the global-operand apply(): same loaded bits, same operation
// order per (token, row, block, lane).

static __device__ __forceinline__ mmvq_yw8 mmvq_lds_preload_w8(const int * rec) {
    mmvq_yw8 w;
#pragma unroll
    for (int m = 0; m < 8; ++m) w.u[m] = rec[1+m];
    w.ds = rec[0];
    return w;
}

static __device__ __forceinline__ mmvq_ywq2 mmvq_lds_preload_wq2(
    const int * rec0, const int * rec1, const int & iqs) {
    const int lane = (iqs/2)%4;
    mmvq_ywq2 w;
    w.u[0][0] = rec0[1 + lane]; w.u[0][1] = rec0[5 + lane]; w.ds[0] = rec0[0];
    w.u[1][0] = rec1[1 + lane]; w.u[1][1] = rec1[5 + lane]; w.ds[1] = rec1[0];
    return w;
}

static __device__ __forceinline__ float vec_dot_iq3_s_q8_1_apply_lds(
    const int * rec, const int * dec, const int scale, const float d) {
    int sumi = 0;
#pragma unroll
    for (int m = 0; m < 8; ++m) sumi = ggml_cuda_dp4a(dec[m], rec[1+m], sumi);
    sumi *= scale;
    return d * __low2float(*reinterpret_cast<const ggml_half2 *>(rec)) * sumi;
}

static __device__ __forceinline__ float vec_dot_iq3_xxs_q8_1_apply_lds(
    const int * rec, const int * dec, const int ls, const float d) {
    int sumi = 0;
#pragma unroll
    for (int m = 0; m < 8; ++m) sumi = ggml_cuda_dp4a(dec[m], rec[1+m], sumi);
    sumi = (ls*sumi + sumi/2)/2;
...
static __device__ __forceinline__ float vec_dot_iq4_xs_q8_1_apply_lds(
    const int * rec, const int * dec, const int scale, const float d) {
    int sumi = 0;
#pragma unroll
    for (int j = 0; j < 4; ++j) {
        sumi = ggml_cuda_dp4a(dec[j + 0], rec[1 + j + 0], sumi);
        sumi = ggml_cuda_dp4a(dec[j + 4], rec[1 + j + 4], sumi);
    }
    sumi *= scale;
    return d * __low2float(*reinterpret_cast<const ggml_half2 *>(rec)) * sumi;
}

static __device__ __forceinline__ float vec_dot_q4_K_q8_1_apply_lds(
    const int * rec0, const int * rec1, const int & iqs,
    const int * dec, const int * sc, const int * m, const ggml_half2 dm) {
    const int lane = (iqs/2)%4;
    const int u0[QR4_K] = {rec0[1 + lane], rec1[1 + lane]};
    const int u1[QR4_K] = {rec0[5 + lane], rec1[5 + lane]};
    const float d8[QR4_K] = {
        __low2float(*reinterpret_cast<const ggml_half2 *>(rec0)),
        __low2float(*reinterpret_cast<const ggml_half2 *>(rec1)),
    };
...
#pragma unroll
    for (int i = 0; i < QR4_K; ++i) {
        const int dot1 = ggml_cuda_dp4a(dec[2*i+1], u1[i], ggml_cuda_dp4a(dec[2*i+0], u0[i], 0)); // SIMD dot product
        const int dot2 = ggml_cuda_dp4a(0x01010101, u1[i], ggml_cuda_dp4a(0x01010101, u0[i], 0)); // sum of u
        sumf_d += d8[i] * (dot1 * sc[i]);
        sumf_m += d8[i] * (dot2 * m[i]);  // multiply constant part of q4_K with sum of q8_1 values
    }
...
static __device__ __forceinline__ float vec_dot_q5_K_q8_1_apply_lds(
    const int * rec0, const int * rec1, const int & iqs,
    const int * dec, const int * sc, const int * m, const ggml_half2 dm) {
    const int lane = (iqs/2)%4;
    const int u0[QR5_K] = {rec0[1 + lane], rec1[1 + lane]};
    const int u1[QR5_K] = {rec0[5 + lane], rec1[5 + lane]};
    const float d8[QR5_K] = {
        __low2float(*reinterpret_cast<const ggml_half2 *>(rec0)),
        __low2float(*reinterpret_cast<const ggml_half2 *>(rec1)),
    };
...
#pragma unroll
    for (int i = 0; i < QR5_K; ++i) {
        const int dot1 = ggml_cuda_dp4a(dec[2*i+0], u0[i], ggml_cuda_dp4a(dec[2*i+1], u1[i], 0)); // SIMD dot product
        const int dot2 = ggml_cuda_dp4a(0x01010101, u0[i], ggml_cuda_dp4a(0x01010101, u1[i], 0)); // sum of u
        sumf_d += d8[i] * (dot1 * sc[i]);
        sumf_m += d8[i] * (dot2 * m[i]);
    }
