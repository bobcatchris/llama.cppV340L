...
- At $448\text{ GB/s}$ theoretical peak GDDR7 bandwidth (256-bit @ 28 Gbps) and $\approx 427\text{ GB/s}$ measured achievable bandwidth on the RTX 5060 Ti:
  $$\text{Theoretical Physical Minimum (Peak)} = \frac{8.0\text{ GB}}{448\text{ GB/s}} = 17.85\text{ ms}$$
  $$\text{Achievable Physical Minimum (Microbench)} = \frac{8.0\text{ GB}}{427\text{ GB/s}} = 18.73\text{ ms}$$
- **Actual Measured Kernel Latency**: GEMV streaming accounts for the bulk of verify time; our isolated Q5 GEMV kernel achieves **~99% of measured achievable peak bandwidth (~423 GB/s)**. Remaining verify latency is consumed by non-GEMV sublayers (Conv1D, GDN recurrence, RMSNorm, RoPE, AllReduce, unpack).
