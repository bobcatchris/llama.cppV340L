...
| `--kv-dtype bf16\|int8\|kvarn_k4v2` | KV-cache storage; `kvarn_k4v2` = quantized K=4-bit V=2-bit tier, the KVarN default | `bf16` |
| `--cache-type-k kvarnN` | override the K-Van K width (`N` = 2..8); only `kvarn4` (the default) is fully served | `kvarn4` |
| `--cache-type-v kvarnN` | override the K-Van V width (`N` = 2..8); only `kvarn2` (the default) is fully served | `kvarn2` |
| `--prefix-cache-capacity N` | prefix-reuse window (tokens); the cached-prefix arena stages up to this many rows | `16384` |
...
### KVarN (quantized KV) tier

`--kv-dtype kvarn_k4v2` selects the KVarN tier: the KV cache is stored as a
per-tile quantized code (K=4-bit, V=2-bit by default) with scale/zero-point
planes, instead of BF16 or int8. It is selected via `--kv-dtype`, and the
rail widths can be overridden independently with `--cache-type-k kvarnN` /
`--cache-type-v kvarnN` (both `N` in `2..8`).

**Width ladder.** The supported/default combo is **`k4v2`** (`kvarn4`+
`kvarn2`). The engine has a width-generic materialize + prefill path that
decodes the quantized codes for arbitrary widths `2..8`, and the full matrix
(rows `kvarn2..8` × `kvarn2..8`) was exercised end-to-end during the docs/70
work. However, the **decode path is width-*specific***: single-token decode
(`tokens==1`, and the D-19 small-T split-K route) reads the fixed `kKvarnKCodeBytes`
(8 KiB, = k4) / `kKvarnVCodeBytes` (4 KiB, = v2) footprints. A `--cache-type-k`
or `--cache-type-v` that is **not** `4`/`2` therefore **emits silently wrong
tokens on the decode path**. The serving layer guards this on startup (it
rejects a non-default KVarN tier at launch rather than serving wrong tokens).
Use only `kvarn4`/`kvarn2` on the server; the wide tiers are a research/matrix
capability, not a serving configuration.

**Staged shadow (the in-capacity wall).** To avoid an O(T×P) re-dequant on
every query for packed pages, the engine stages a transient BP16 shadow of
the KV history *up to a budget* (`kKvarnStagedBudgetBytes`, currently **1344
MiB/rank** ≈ **632 pages** ≈ **~40.4k-token wall** for `k4v2` at the training
page size). Requests whose history fits the shadow stay on the fast
in-capacity (delta-only) path; requests past the wall fall back to the
beyond-wall flash/read path. Prefix-restore requests that land inside the
wall are also delta-only. The wall was raised (1024→1344 MiB, commit
`98b5395e`) so MTP verify rounds up to ~40k stay in-capacity.

**Prefix-reuse window & VRAM.** `--prefix-cache-capacity N` (default
`16384`) bounds the cached-prefix working set. The prefix-reuse path stages
the open-page BP16 tails + MTP seed at save time and restores them on a full
hit (S). The GDN checkpoint buffers add a per-rank cost (~73.4 MiB/rank at
the 48k working max). **VRAM caveat (D-20):** the startup pre-check
under-estimates prefix-capacity cost — a 98k context OOM'd after a pre-check
PASS, while 48k was the working max. Validate a prefix-reuse-heavy config
against actual free VRAM at startup, not only the pre-check.
