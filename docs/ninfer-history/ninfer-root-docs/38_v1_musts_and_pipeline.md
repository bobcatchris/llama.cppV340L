...
## 7. v1 Final Sign-Off Status (2026-08-21)

All v1 Musts (M1–M7) are **100% COMPLETE & EMPIRICALLY VERIFIED**:
1. **M1 (Multi-request stability)**: Fixed via arena scoping + per-request reset; 10/10 mixed requests pass with 0 MiB VRAM growth.
2. **M2 (Full sampling suite)**: Temp, top_k, top_p, min_p, presence/frequency penalties, seed all wired end-to-end; TC-S sampling tests pass.
3. **M3 (Option plumbing)**: CLI options aligned; tokenizer stop tokens registered.
4. **M4 (Quantized INT8 KV Cache)**: Fully functional; 64k context decoder state fits in 1.31 GB/rank.
5. **M5 (Prefix Caching default ON)**: Implemented via GDN slot snapshot/rebase (<0.1 ms); multi-turn chat verified.
6. **M6 (Prefill & context capacity)**: Preflight bounds allocations; 64k context executes end-to-end.
7. **M7 (Standing gates)**: `verify_battery.sh --build` passes 100% (0 fail, 0 warn) at 93.45 t/s MTP k=3.
8. **Comprehensive Report**: See [`39_v1_results_and_signoff.md`](39_v1_results_and_signoff.md).
