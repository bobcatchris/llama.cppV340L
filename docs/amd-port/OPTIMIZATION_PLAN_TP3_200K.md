...
- E-130 SOAK-MECHANISM DESK PHASE 1 COMPLETE (ZERO-GPU; wt-soak on
  amd/soak-mech; receipt W18_soak_mechanism_receipt_2026-09-24.md +
  W18_soak_timeline_2026-09-24.txt + instrument
  docs/amd-port/scripts/soak_timeline.py). THE PHENOMENON IS NAMED: FAR-DIE
  SOAK - thermal sclk throttling of the two far-board dies (0D:00 + 10:00),
  not HBM refresh, not driver state, not board power sag. (1) DECAY
  TIMELINE (E-128 hardened boot, identical code p0a/p0b): decode 23.33 ->
  17.35 (-26%) while prefill 214.59 -> 211.57 (-1.4%); the decay is
  decode-specific. Battery structure mapped: decode_guard runs at ~t+30-90 s
  of each cell; the 127-208 s 200k needle_recall guard at the cell END is
  the soak source for the NEXT cell; the ~96 s inter-cell gap leaves the
  card at junc 47-62 (vs 27-35 fresh) and never clears the soak. Die 0D:00
  latches into a 991/775/560 MHz band mid-cell (p0b from ~91 s, never
  recovers in-cell) while 05:00/08:00 hold 1138-1500 in every cell. W15's
  4-die log confirms the class split at steady state: 0D:00/10:00 at
  788-980 for 47 min while 05:00/08:00 hold 1094-1465 at junctions FLAT
  83-85 on all four dies - junction does not discriminate, clocks do.
  CORRELATION OF RECORD (18 cells): tps 23.2-23.6 iff die-3 sclk act-mean
  >= ~1190 with <= 991 MHz duty ~0-20%; tps 17.3-17.7 iff act-mean
  1068-1135 with duty 44-53%. KEY CELL CLASS: hot-but-unthrottled 10k cells
  (run 2-3 min after a 200k battery, mem 84-88 C) decode 21.8-22.5 at FULL
  clocks - hot alone is not slow, throttled is slow. (2) SENSOR CENSUS:
  every die exposes edge (crit 85 / emerg 90 C), junction (105/110), MEM =
  HBM temp (95/100 - the channel exists and has been logged all along,
  never before used), sclk/mclk freqs, vddgfx, PPT (cap 110 W/die = AT
  cap_max, unraisable), fan pwm (auto, no usable tach). pp_dpm/pp_od files
  exist but read EMPTY unprivileged (root-gated). Not exposed on gfx900:
  VRM temps, per-stack HBM temps, throttle-reason (gpu_metrics is Navi+).
  (3) VERDICT + KILLS: driver-state decay REFUTED (10k cells at 22.5 decode
  full speed on a soaked card minutes after a 200k battery; s0b 23.51 after
  a ~13 min idle gap; E-127 pos-1 23.30 on an 8 h boot - the state variable
  is thermal, clears on idle in minutes, no reboot needed); pure-HBM-refresh
  REFUTED as primary (same cell class) but survives as a contributor at
  deep soak (mem peaks 87-95, one 95 sighting vs crit 95); power sag has no
  direct evidence (PPT never logged - gap). The throttle trigger is NOT the
  reported junction (flat 84-91 on throttling AND non-throttling dies);
  edge crit = 85 C matches die-3 pin onset (edge 84-85 whenever latched)
  but near dies at the same edge do not pin - per-die limit/airflow
  asymmetry; an unexposed VRM/board sensor is the likely true trigger.
  HONEST BOUND: sampled sclk shrink explains the bulk of -26% but 5 s
  aliasing + unlogged mclk/PPT could hide a bandwidth-side residual.
  (4) DISCRIMINATORS for the instrumented session: (a) one 1 s cadence log,
  all FOUR dies (sampler currently misses die 4 entirely - card0/1/2 only),
  sclk+mclk+PPT+vddgfx+temps: mclk-at-max-while-sclk-pins => clock policy
  case closed; PPT-pinned-110 => power limit; mclk-down => bandwidth term.
  (b) root-gated clock-floor arm (pp_od_clk_voltage/pp_sclk_od, ~1200 MHz
  floor, decayed window): t/s restores to ~23 => mechanism confirmed;
  no change => bandwidth term owns it. (5) SERVED-ARM SPEC / CHEAP FIX
  (coordinator): bank decision cells at pos-1 or after >= 5-10 min idle -
  the 96 s gap is measured insufficient, ~13 min sufficient (mechanistic
  upgrade of the E-119 soak law); OBJECTIVE VOID GATE computable from the
  existing sideband: any cell with die-3 sclk act-mean < ~1150 or <= 991 MHz
  duty > 20% in its decode window is VOID (retroactively applicable);
  shorten or reorder the end-of-battery 200k needle guard (it soaks the
  card for the NEXT cell); physical owner-side fix = airflow to the far
  dies (3-8 C hotter at idle); no software fan lever; PPT cap unraisable.
