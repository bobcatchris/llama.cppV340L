...
- E-132 (cont.) ZCODE AUTO-LAUNCH WIRED (Chris: "zcode has to be
  ...
  resume.
- E-133 2026-09-24 06:35 HARDENED PAIRED QUEUE BANKED + SOAK MECHANISM NAMED + FLEET RECONCILED + ARM-IDENTITY GAP CLOSED. (a) HARDENED QUEUE (boot 05:16, noretry=1 verified live, 4/4 PCI dies present) ran the paired prefill queue p0a/p1a/p0b/p1b 05:31-05:50, provenance clean on all 4 cells (commit ab2c44c5d, binary 938a6999, tree clean, stamped). VERDICT: ub1024 prefill win REPLICATES IN-PAIR (+4.0% p1a-p0a: 223.25 vs 214.59; +5.0% p1b-p0b: 222.12 vs 211.57); decode penalty -14.6% in the fresh pair, ~0 in the decayed pair (soak interaction observed; decision UNCHANGED: rejected for decode default, one-line flag for prefill-dominated profiles = L4 stands).
...
- E-134 2026-09-24 07:40 DEATHS #4 AND #5, CRASH-LOOP BROKEN, SVM-HOG WATCHDOG, SOAKED-BOOT PAIRED DATA, OBJECT-PURGE #2. (a) DEATHS: the 05:16 boot died hard at 06:45 (~90 min up; per-boot svm_range_deferred_list_work hog counter reached 5, last escalation 06:42:45 exactly as the dc window's d1 server was booting); the 06:46 boot died at 07:04 after ~19 MIN (counter reached 4 at 07:00:44) - noretry=1 did NOT prevent either death. Deaths cluster at llama-server boot/teardown cycles (KFD SVM allocation churn). The enabled postreboot self-fire turned this into a CRASH LOOP (boot -> queue fires within 600 s -> churn -> death in 20-90 min -> reboot); loop broken this era by the watchdog below.
- E-134 (cont.) (b) SVM-HOG WATCHDOG INSTALLED (/home/chris/svm_watchdog.sh + svm-watchdog.timer, minutely, root): reads the per-boot kernel hog counter; at >= 3 (deaths observed at 4-5) it CLAIMS /tmp/campaign_gpu_boot.lock, which every queue/window/desk already waits on - GPU pipelines pause at the NEXT cell boundary instead of churning into death; releases when the counter is < 3 or unchanged for 10 min. (c) CHURN SPACING: inter-cell settle 75 -> 180 s in run_combined_window.sh + run_postreboot_hardened.sh. The REAL churn cut - a persistent-server battery protocol (one server boot, N batteries) - is named and flagged for OWNER decision (it changes the measurement protocol).
- E-134 (cont.) (d) GIT OBJECT PURGE #2: the 07:04 death emptied the fa40 agent's just-written WIP objects (amd/fa40-codegen tip c47407a1 + wt-fa40-cg index). Coordinator recovered: ref reset to intact 66679ceac (variant pack), worktree index rebuilt, empty objects purged, fsck clean; the agent's UNCOMMITTED edits (bench_attn_real.cu, fattn-tile.cuh) survived on disk. fa40 desk re-dispatched (v3) with crash-recovery brief. REPEATED LESSON: every hard death risks git objects - snapshot pushes after every landing are load-bearing.
- E-134 (cont.) (e) BOOT #3 (07:05) QUEUE = THERMALLY INHERITED DEEP SOAK: p0a 17.60 / p1a 17.73 / p0b 16.72 / p1b 16.85 t/s decode - the whole boot sits in the soaked ~17 t/s regime from position 1 because boot #3 started ~60 s after boot #2's death (dies still at mem 84-95 C; the ~13 min passive-recovery evidence from W18 applies). Paired ub1024 deltas +0.13/+0.13 = soak noise (the morning's -14.6 percent fresh-pair ub1024 decode penalty and +4-5 percent prefill win stand; L4 verdict unchanged). LAW REFINED: a boot that follows a crash has NO fresh-boot position-1 control until the dies cool; the die_hot<60 gate now enforces cooldown for windows (the hardened queue lacks it - added to the next revision). (f) DC WINDOW: first launch (root-run systemd) was lost to death #4 mid-d1; its r1 control (fresh merged binary, decode 23.33, boot #1 pos-1) is banked as SUPPORTING evidence only - its provenance stamp read commit=unknown under root (safe.directory), the exact blindness the dc desk fixed. Window relaunched as User=chris. (g) BATTERY OBSERVABILITY: the hb queue's grep pipe swallowed the p0a failure mode entirely (613-byte cell files, no error) - run_postreboot_hardened.sh now tee's the full battery output to <cell>.battery_full before filtering.
- E-135 2026-09-24 08:25 DC WINDOW ATTEMPT #3: THE ENGAGED SHAPE-CACHE ARM CRASHES - REAL BUG FOUND, FIX DESK DISPATCHED; TWO TOOL DESKS LANDED; WATCHDOG+GATE HARDENED. (a) WINDOW OUTCOME: r1 FAIL boot-timeout + d1 hollow - BOTH were permission-denied writing their own cell logs: the 06:42 dead-boot chain ran as ROOT and left root-owned combowin_r1/d1 files in /home/chris (138 root-owned campaign files total; chowned to chris, stale d1 parked .deadboot_root). LESSON CODIFIED: never mix root-run and user-run window outputs in /home/chris - chown after any root-run window. (b) r2 (control, pos-3) PASSED decode 23.43 (+0.88 percent, provenance clean, void-gate-valid): the dies RECOVERED from the boot-3 morning soak after ~30 min idle - W18's passive-recovery model confirmed served. (c) d2 (LLAMA_DRAFT_SHAPE_CACHE=1) ENGAGED CORRECTLY - "draft shape cache enabled (2 slots)" printed exactly twice (the E-133 INFO->WARN fix works served) - and then the server ABORTED mid-battery: ggml/src/ggml-backend-meta.cpp:1929 GGML_ASSERT(bcj.nodes[i]) failed, stack ggml_backend_meta_graph_compute <- process_ubatch <- llama_decode <- common_speculative_impl_draft_mtp::process. THE OLD "draft-cache no-win" VERDICT (E-126) WAS MEASURED ON AN INERT ARM; THE REAL ARM DOES NOT JUST LOSE, IT CRASHES THE DRAFT PATH. Shape-cache lever CLOSED until fixed; dc-crash-fix desk dispatched (zero-GPU: root-cause the draft-path replay null node, fix fail-open or key-complete, host-verify, report on lever viability).
- E-135 (cont.) (d) TOOL DESKS LANDED (both zero-GPU, committed to main): VOID-GATE adjudicator (docs/amd-port/scripts/void_gate.py, 945118839 + a35fced8e) - classifies any banked cell PASS/VOID/UNSUPPORTED by the E-133 die-3 sclk rule; validated on 15+ banked cells (boot-3's four cells all VOID from pos-1, fresh cells PASS, W18 numbers reproduced exactly; gray band disclosed: mid-soak cells can squeak past - the gate is a backstop, pos-1 scheduling law stands). PERSISTENT-SERVER protocol (docs/amd-port/scripts/run_persistent_battery.sh + docs/amd-port/results/W21_persistent_server_brief_2026-09-24.md, b45f5499e + 57018f7c7): boot-once-per-arm N-cells with slot-erase between (POST /slots/0?action=erase, needs only --slot-save-path - ZERO server code), scrub cell first, arms sequential not interleaved; churn per window drops 2->1 (combined) / 4->2 (hardened paired); owner decision pending: approve tiering (persistent = SCREENING, classic interleaved = ARBITRATION) - brief section 8. (e) COORDINATOR TOOLING FIXES: svm-watchdog deadlock fixed (release path was unreachable at count >= 3 - counter never decreases within a boot; now releases after 10 min quiet); the window gate's binary-freshness check now keys on the last CODE commit (ggml/src src tools common), not HEAD - docs-only commits no longer trip it. (f) WATCHDOG LIVE FIRE: boot-3 hog counter reached 5 (death threshold) at 08:09 - the watchdog claimed the lock, held all pipelines through the danger window, and released on quiet; machine survived. (g) OPEN: dc-crash-fix desk running; fa40 desk v3 building its oracle bench (silent since ~07:30 - nudge/redispatch by 08:35 if no artifacts); r1/d1 rerun queued behind the crash fix (the pair is moot until the arm survives); svm hogs 5 on boot-3 - watch.
- E-136 2026-09-24 09:05 FA40 PROMOTED TO SERVED WINDOW - THE MISCOMPILE IS ROUTED, THE DEEP LEVER IS REAL. (a) ORACLE VERDICT (W17, amd/fa40-codegen 6e042376e+b2bbf9977, desk v4; die 3, dst memcmp vs f16-KV base): variant v11 (scalar half2 stores) BIT-EXACT at d=7168/10240/65536/199936 - staged KT/VT AND the KQ accumulator identical to the reference. ALL OTHER variants (8B shared-tile stores, every restructure: noinline/volatile/bounds/unroll/half-K/optnone) produce DUST (6144/6144 els garbage) diverging at the FIRST stage - the shared K tile is corrupt immediately after load. THE MISCOMPILE IS LOCALIZED to the ggml_cuda_memcpy_1<8> shared-tile STORE granule; scalar stores are the only exact shape. Desk recommendation adopted: stop sweeping; var 11 is the library arm (GGML_CUDA_FATTN_TILE_Q40_DIRECT already instantiates it).
- E-136 (cont.) (b) TIMING (v11 vs f16-KV in-arm control, solo, 30 iters): -37.5 percent per launch at serve depth 7168; -39.1 at 10k; -38.5 at 65k; -39.4 at 200k (rerun -34.7, min-to-min -45.5). Ratio held in EVERY window (thermals drifted absolutes +12-15 percent, die went j85/mem88 hot - the 200k spread up to 26 percent exceeds the W11 law in that state; round-level confirmation at 200k belongs to a quiet-boot window = U1). The instance is latency-bound, not DRAM-bound: the modeled 15.2x byte reduction lands as ~35-40 percent wall, roughly flat in depth. (c) MERGED to main (f60f80b45 keep-both, bench harness conflicts all empty-HEAD theirs-adds), rebuilt (BUILD-EXIT:0, gate string live in libggml-hip), CI PASS. Gate line INFO -> WARN (5b955c385) so the engagement proof is greppable served - same served-filter class as E-135's dc fix. (d) SERVED WINDOW LAUNCHED (campaign-fa40-window, User=chris): interleaved r/fa x2 pairs, 10k decode-only cells, engagement grep per direct cell, void-gate on adjudication. Short-depth served delta expected SMALL (tile kernel is a minor slice of short decode); the DEEP delivery is U1 (see below). Ratchet only on a paired served win per law. (e) U1-PREP DESK LANDED (726774f1d + /home/chris/run_u1_window.sh, dry-run verified): thermal sampler now keys on the 4 die PCIs via sysfs (18-col layout, die 4 sampled - it LIVE-CAUGHT die 4 at mem 92 C during the fa40 bench, the exact blindness class; pp_dpm_sclk parse case-insensitive; legacy 14-col fallback declared). run_u1_window.sh: one boot, depth cells 10k/50k/100k/150k/199k at real depth (cache_prompt=false, n_predict=128 temp 0), per-cell PROVENANCE + verdict + in-log void-gate, 780 s soak settles, wall 7-14 h (prefill-dominated). U1 = screening tier by protocol; owner tiering decision still pending (W21 s8). NOTE: void_gate.py's --observe --json trailing-text bug (raw_decode fix) was found by the desk in testing but is NOT in the committed file - re-fix before using --observe --json.
- E-136 (cont.) (f) FLEET: crash-fix desk still root-causing the shape-cache draft-path null node; fa40 v4 COMPLETE (this entry); U1-prep COMPLETE (this entry). svm hogs: 5 steady on boot-3, no escalation since 08:09, watchdog released and armed; fa40's die-3 sessions and the window run under the watchdog's pause protection.
- E-137 2026-09-24 09:30 FA40 VAR-11 PROMOTED: PAIRED SERVED WIN + RATCHET - THE DECODE OF RECORD IS NOW 24.55. (a) SERVED WINDOW (campaign-fa40-window, 10k decode-only cells, User=chris, provenance clean all 4): PAIR 1 (both cells void-gate PASS, dies clean): fa1 24.55 vs r1 23.38 = +1.17 t/s (+5.0 PERCENT). PAIR 2 (both cells VOID - the boot's soak set in by minute 16, act-mean 1121/1133): fa2 24.29 vs r2 23.28 = +1.01 (+4.3 PERCENT), directionally consistent but not decision-grade. Engagement confirmed served (the WARN gate line prints per graph build, 320/cell). Short-depth served gain was predicted small and is NOT - the tile attention kernel is a bigger slice of short decode than modeled. Combined with the W17 oracle (-37.5 PERCENT per launch at serve depth, bit-exact) this is a validated promotion: ONE clean positive pair is the bar; reinforcement is threefold (pair 2 direction, oracle, per-launch math).
- E-137 (cont.) (b) RATCHET EXECUTED: baseline_tp3_200k.json decode_per_second 23.23 -> 24.55 (clean pair-1 cell; warn -5 / fail -10 thresholds unchanged). GGML_CUDA_FATTN_TILE_Q40_DIRECT=1 added to the BASEENV of EVERY campaign runner (run_combined_window.sh, run_postreboot_hardened.sh, run_u1_window.sh, launch_tp3_200k.sh) - the q4_0-direct arm IS the new serving of record; the E-105 lineage numbers (23.26 @200k / 23.21 @10k short-prompt) are SUPERSEDED. The postreboot self-fire queue on the next boot serves the new config and gates against the new anchor - consistent. (c) CRASH-FIX v2 desk ALIVE (its first-dispatch twin died silently): at the crash site - 1929 sits in a needs_rebuild loop; the lookup misses the stc_compute[stc_compute_index] map; reading alloc/init_tensor flow. (d) NEXT: clean reboot before the U1 window (boot-3 at 7 cumulative hogs; U1 wants a 7-14 h stable boot with a zeroed counter and cool dies). U1 now measures the new of-record env, so its depth table IS the promoted config's depth table. The deep prize stands: -37.5 PERCENT per launch on the tile kernel at depth where attention is ~386 ms of the ~503 ms round.
- E-138 2026-09-24 10:05 SHAPE-CACHE CRASH FIXED + MERGED - REBOOT INITIATED FOR THE U1 ERA. (a) ROOT CAUSE PROVEN (dc-crash-fix v2, amd/dc-engagement a7a1f0fda, merged clean 21431b232): the shape cache re-enters a PERSISTENT built graph (src/llama-context.cpp:1613-1671 re-entry branch); ggml_gallocr_init_tensor skips tensors with data != NULL (ggml-alloc.c:969-994) so init_tensor never re-runs for the reused graph; each meta compute with a changed uid CYCLES the double-buffered simple-tensor containers and clears the other one (ggml-backend-meta.cpp:1900-1913), destroying the re-entered graph's registrations; the rebuild mapping then reads NULL (1928) -> GGML_ASSERT(bcj.nodes[i]) at 1929. The DRAFT-MTP ctx is the first victim because its catchup/step shapes alternate decode-over-decode (constant re-entry); the target path's single stable shape never re-enters. (b) FIX (25 lines, one file, fail-open): ggml_backend_meta_buffer_simple_tensor lazily RE-REGISTERS a missing entry instead of returning NULL - the assert is unreachable by construction; the uid memo still hits after healing so device-graph replay (the cache's actual win) is preserved; heals converge to zero steady-state; rate-limited WARN names each healed tensor for served greppability. Default-off untouched. HOST-PROVEN: docs/amd-port/tests/test_meta_reentry_host.cpp (CI section 2c) reproduces the abort PRE-fix and ALL-PASSes POST-fix with convergence; CI 10/10 PASS; rebuild BUILD-EXIT:0. (c) VERDICT: the ~3 ms shape-cache lever SURVIVES FOR BOTH PATHS - the defect was bookkeeping, not the cache key. The W19 spec re-run (interleaved r/d pairs, engagement == 2, now plus re-registration WARNs at first alternation) is queued AFTER the U1 era. (d) REBOOT INITIATED (U1_ARMED set): chain on the fresh boot = postreboot queue (serves the NEW of-record env GGML_CUDA_FATTN_TILE_Q40_DIRECT=1, gates vs the ratcheted 24.55 anchor) -> U1 depth window (10k/50k/100k/150k/199k, one persistent server, 7-14 h, per-cell void-gate). Boot-3 retired at 7 cumulative hogs, quiet since 08:59, ZERO deaths on the watchdog's watch - the pause machinery held. Serving of record entering the reboot: decode 24.55 short-prompt (fa40 var-11 + TP4 + RCCL + gates, binary lineage this merge's build), deep table pending U1.
- E-139 2026-09-24 11:45 BOOT-4 QUEUE 4/4 PASS + UB1024 CONTRADICTION FOUND + W22/W23 DESKS LANDED + U1 MARCHING. (a) QUEUE (fresh boot, all provenance-clean): controls p0a 214.34/24.45, p0b 212.94/23.78; ub1024 p1a 224.60/24.73, p1b 224.15/24.64. THE UB1024 CONTRADICTION: on the fa40 config, ub1024's decode penalty has VANISHED - paired raw deltas +0.28/+0.86 t/s decode AND +4.8/+5.3 PERCENT prefill (both pairs positive on both metrics). The E-127/E-133 rejection (-14.9 PERCENT fresh-pair) was likely soak-confounded or fa40-interacted; NEW EVIDENCE overrides the standing verdict per law, but promotion needs a dedicated paired window - QUEUED behind U1. If it holds: decode ~24.7 + prefill ~224. (b) W22 DIE-ASYMMETRY (67b75992b+2e27d757b+bb3fbec77): cause = CLOCK/POWER STATE - host NUMA/affinity refuted (single-NUMA, enqueue 0.2 us no-op, W10 multiplicative device law), transport excluded (W9 flat isolated allreduce), channels closed served (chan4 E-119). Die-3 is the slowest COMPUTE die (t_d 0.893/0.846/0.964/1.000); W8 medians are wait redistribution. RANK 1: per-die clock floors (root: power_dpm_force_performance_level=high, or pp_od s 0 1200) - full t_d equalization ~+4.8 PERCENT, halving ~+2.4, PLUS W18 anti-decay; kill: paired < +0.5 t/s or clocks pinned without t_d compression (closes the lever). Rank 2: taskset P-core pinning (0-1 PERCENT class). Spec W22 has exact window deltas. (c) W23 MMVQ-BW (33ea4db13): gap = STALL-BOUND LATENCY STARVATION, not bandwidth - 1-2 kbx iterations/lane at K=5120, register-capped occupancy (8/40 waves, __launch_bounds__ 1), cfull ablation ceilings 70-120 GB/s = the access-pattern ceiling; served types IQ3_S/IQ4_XS/IQ3_XXS/Q4_K/Q6_K (3.08 GB/die, matches of-record). CANDIDATES: C1 re-open IQ4XS_SHARE (env-only, -2.0 ms; E-104 disabled it on stale replica numbers), C2 SET IQ3XXS_S2R=1+IQ3S_S2R=1 (env-only, -1.4 ms; the E-117 fix landed but NO serving script sets these), C3 wide-s2r y-preload (new arm, -2.5..6 ms, bit-exact by construction), C4 lift GLU fusion ban to T=4 (-2..4.5 ms, code change). (d) U1 LIVE: first cell u1_10k MEASURED (void-gate PASS, die3 1204 MHz, draft 0.68); new die4-pci-indexed sampler working; 780 s soak settles between cells; 7-14 h run underway. (e) POST-U1 EXPERIMENT QUEUE (ordered): W23 C1+C2 env window (cheapest, env-only), W22 C1 clock-floor window (root pre-boot), ub1024 re-promotion window, W19 shape-cache re-run. svm hogs 5 on boot-4, watchdog armed.
- E-139 (cont.) 2026-09-24 12:55 W25 NULL + W27 STAGED + EOS FIX. (a) W25 WIDE-S2R (amd/wide-s2r, 94792f074..2f2ade75a): oracle PASS 5/5 types (iq4_xs/q4_K/iq3_s/iq3_xxs/q5_K, base-dup drift controls), timing NEUTRAL - the compiler already hoists the y-preloads (+0..3 regs, CTAs/CU unchanged); W23's -2.5..-6 ms estimate REFUTED; GGML_CUDA_MMVQ_WIDE_S2R stays default-off, honest null banked with admissible-rep spread law. (b) W24 PREFILL-AT-DEPTH (4ddcdd746+9f0f35965): collapse closes ZERO-FREE-PARAMETER as 3.73 + 0.42*D/1000 ms/token; quadratic term legitimate (same per-row latency as decode); excess = 7.1x f16-pool read amplification inside the quadratic scan; GO verdict for the prefill q4_0-direct extension; prize 22->34 t/s prefill @199k. (c) W27 Q40-PREFILL IMPLEMENTED (amd/q40-prefill, 0d973076a+c472fa1d2+9af2edc3a): new env GGML_CUDA_FATTN_TILE_Q40_PREFILL as a disjoint T>4 gate beside the (byte-identical canary) decode gate; var-11 at the <256,256,8,2> prefill instance with need_f16=false; bench instrument gains runtime M + --used (padded causal boundary) + prefill arms; BUILD-EXIT:0; oracle STAGED (one command, run_q40prefill_bench.sh, PCI-resolved - note die 0d:00 is HIP index 2 on boot-4, the script is immune). U2 served sequence spec'd with kill criteria (< +5% @10k, GARBAGE, or decode_guard breach). (d) U1 EOS BUG: u1_50k decode died at 1 token - temp-0 EOS immediately after the depth filler (draft_n=0, sentinel tps); 50k PREFILL point valid (29.41 t/s @51203). Fixed in run_u1_window_v2.sh (ignore_eos:True); the live script cannot be edited mid-run - deep decodes face the same coin flip; deep-cell rerun with v2 if >=2 die. 100k cell in fresh ~102k prefill (LCP line verified slot-choice-only, timing honest). svm hogs 5, quiet, watchdog armed.
- E-139 (cont.) 2026-09-24 13:05 CROSS-PROJECT ACCEPTANCE SYNTHESIS ADOPTED (owner brought the ninfer wo/kv4-cpu receipt, 103 rows: "MTP Acceptance - what actually moves it"). LAWS ADOPTED INTO OUR MEASUREMENT PRACTICE: L2 acceptance resolution ~+/-3 pts - single-cell draft_accept deltas are not evidence; paired interleaved cells mandatory, kill criteria quantified vs the band. L3 trajectory divergence - a numerics change swaps in a DIFFERENT text with a different per-text head-agreement rate; served draft_accept deltas from quant changes are population draws, NEVER pairing-quality claims (teacher-forced fixed-text instrument is out of serving scope). L8 pairing lottery - non-bit-exact numerics changes (share gates, ub1024) move acceptance as a lottery ticket; bit-exact arms (fa40 var-11, S2R) are acceptance-neutral by construction. L4 CONTEXT IS THE BIGGEST ACCEPTANCE LEVER (+25 pts 10k->72k on their stack) - REGISTERED PREDICTION: U1's deep cells' draft_accept should climb above the 0.66-0.68 short band if the law transfers to llama.cpp draft-mtp; the U1 live-analyst tracks acceptance-vs-depth in W26 as a falsifiable tonight-test; if it holds, deep decode outperforms the E-124 law (which models ms/round only). NOT TRANSFERRED: head re-fit (our head ships in the GGUF - no served-side lever; if the draft KV is already f16, acceptance is head-calibration-bound and the W28 verdict will say so), TP2-vs-TP1 (we are TP4), rebake specifics. ROUTED: W28 desk briefed (laws into the A4 analysis + quantified kill criteria); U1 live-analyst briefed (acceptance-vs-depth section + registered prediction in W26). Owner's ranked levers for the record: context > head re-fit > topology > MTP-block format > backbone-format rebakes (lottery).
- E-139 (cont.) 2026-09-24 13:35 W28 MTP-ACCEPTANCE CENSUS - OWNER HYPOTHESIS ANSWERED, CHAIN-SHAPE LEVER SPEC'D AND WIRED. (a) PRECISION CENSUS: the draft-mtp draft context's KV is ALREADY F16 - the -ctkd/-ctvd defaults are GGML_TYPE_F16 (common/common.h:340-341) and -ctk/-ctv reach only the target (common/common.cpp:1602-1603); no inheritance either way. The owner's "-ctkd f16" window is moot (A/B of two identical arms). Acceptance is HEAD-CALIBRATION-BOUND: the MTP head is a 1-layer predictor trained against the 65-layer backbone (GGUF-bound, no served lever); the verifier's q4_0 noise is SHARED conditioning (cancels, not a draft/target gap); sampling clean (temp 0 + bit-exact fast paths). LLAMA_DRAFT_ONDEVICE_ARGMAX carries an exact-equivalence contract (src/llama-ext.h:113-120) - bandwidth lever only, acceptance-neutral. (b) ENVELOPE CORRECTION: dies are 8 GiB (8,576,157,376 B), NOT 16 - current usage 6,978/8,176 MiB per die under the U1 server (~1.2 GiB/die free). Draft ctx caches ONE nextn layer only (qwen35 hybrid filter, 16 of 65 layers carry KV): 195.3 MiB f16 at 200k (q4_0 would be 54.9 - quantizing it saves 140 MiB for <0.1% decode: NOT RECOMMENDED, L8 lottery for nothing). (c) THE LIVE LEVER = CHAIN SHAPE: --spec-draft-n-max 4 (real flag, arg.cpp:3738) extends proposals to 4 tokens; mean_len 3.04 -> ~3.6+ if acceptance holds => up to ~+20 PERCENT decode. Wired as the 5th window (w28nmax) in run_post_u1_battery.sh: R A A R 10k cells, acceptance + cross-arm sha adjudication (greedy verify is draft-shape-invariant - text mismatch = hard VOID, the content-invariance law), promote only if paired WIN with accept inside the L2 +/-3 pt band, hard VOID below 0.63 canary or >5 pts under control. Dry-run verified (battery correctly blocked while U1 holds the machine). (d) U1 STATUS: 100k cell prefilling (dies 91 C under load, watchdog armed); W26 depth table draft live; acceptance-vs-depth registered prediction pending the deep cells.
- E-141 2026-09-24 13:55 W29 PP-vs-TP RE-EVALUATION - NO-GO x2, CLOSED BY DERIVATION (zero GPU; receipt docs/amd-port/results/W29_pp_reeval_2026-09-24.md). (a) ARCHAEOLOGY: the early PP era (2026-09-23 06:36-06:38, binary b739aeb0d = E-102, TP3 machine) NEVER MEASURED PP DECODE - the -sm layer server died at decode entry ("Remote end closed connection", pp3_layer.jsonl FAIL) after a -63 PERCENT prefill (60.39 vs 163.06 t/s); E-107's "layer-split (dead, crash + -63 percent prefill)" was an anecdote, so the owner's re-open was legitimate. (b) MISSION PREMISE CORRECTED: the boundary wall is 29-31 ms of the ~124 ms MTP CYCLE (24-25 PERCENT; 136 boundaries, die medians 128.8-210.8 us, W8), NOT 70-75 percent - the 70-75 framing divided per-cycle NCCL ms by the per-token wall (1000/24.55 = 40.7 ms). The round is ~65 PERCENT per-die compute (MMVQ stream + flash + small kernels, C ~ 80-85 ms/cycle paid CONCURRENTLY at width 4), ~25 PERCENT allreduce, ~10 PERCENT host/draft/gaps. (c) PP4 KILL MATH: -sm layer removes the 31 ms boundary wall but serializes compute width 4 -> 1: cycle = 4C + ~25 = ~350-365 ms => 8-9 t/s vs 24.55 of-record (-63 to -68 PERCENT); to WIN, PP4 needs C < 25 ms = a 3.3x per-die compute gain the banked physics forbids (MMVQ stream alone is 3.06 GB/die at the W5 70-120 GB/s access-pattern ceiling = 25-44 ms). Kernel progress SHRINKS C, which makes PP monotonically WORSE. Draft-mtp interaction traced: nextn + output head land on the LAST stage, so draft steps do NOT traverse stages (0 x 4, not 6 x 4); the chain cost is unsharded compute +10 ms/cycle - noise vs +3C. Hybrid KV placement is PP-FRIENDLY (attn layers at (il+1) percent 4 == 0 = il 3,7..63 + nextn; any contiguous 16-17 layer stage holds exactly 4 KV layers, max 5; stages own their KV, zero cross-stage traffic) - placement is NOT the problem, serialization is. Capacity: PP4 stage 3 @200k = ~8.3-8.4 GB vs 8176 MiB (nextn KV f16 unsharded 0.82 GB + lm_head q6_K 0.58 GB unsharded + weights) = OOM RISK; 10k fits. (d) tp2xpp2: NOT IMPLEMENTABLE (no composable TPxPP mode in the tree: -sm tensor shards every layer across ALL backends, src/llama-model.cpp:316-319; -ts is a per-GPU weight fraction). Hypothetical boundary count does NOT halve (still 2 cuts/layer within each TP2 pair, 130/pass, just cheaper n=2 ops ~110-160 us): cycle ~199 ms = ~15.4 t/s (-38 PERCENT), and TP2-class capacity CANNOT BOOT at 200k (E-032). (e) REOPEN CONDITIONS REGISTERED (all four in receipt section 6): boundary wall > 250 ms/cycle; C < 31 ms/cycle; true multi-ubatch bubble-free PP; owner prefill pivot with new data. OPTIONAL single falsification cell registered PRE-BLIND (10k-only -sm layer boot, predicted 8-9 t/s, kill bar > 15 t/s reopens; NOT scheduled). (f) Served-frame correction for the record: the real point of slowness is ~65 PERCENT weight-stream compute (W5 ceiling) + ~25 PERCENT allreduce latency; the open levers of record (W23 C1/C2 env window, W22 C1 clock floors, W8 transport asymmetry, host slice) already target the big terms - no structural re-aim. U1 lock untouched; u1_depth_100k had not landed at desk close (E-124 law projection ~303 ms/cycle used for depth math; U1 cells will confirm).
- E-142 2026-09-24 14:10 EXTERNAL REVIEW ADOPTED - TWO NEW DESKS DISPATCHED, BATTERY CONFIRMED AS THE VEHICLE. The owner relayed an outside review of the dossier; triage: (a) ALREADY STAGED, CONFIRMED: W23 env flips (window 1, ~-10 ms/cycle at 3.04 tokens), clock floors W22-C1 (window 4, +2.4-4.8 percent), chain shape n_max=4 (window 5 - the review's headline math: 3.65 tokens/cycle at UNCHANGED 124 ms wall = 29.4 t/s; our window measures exactly this with the content-invariance sha check), persistent-server W21 (review endorses as the death-class cure; Phase-1 shadow queued post-battery). (b) NEW LEVERS, DESKS DISPATCHED NOW: W30 MMVQ-C4+OCCUPANCY (wt-mmvq-c4, amd/mmvq-c4): the GLU fusion T=4 lift (-9..-13 ms/cycle claimed; W23 said -2..-4.5 ms/round - measure it) PLUS the register-clamp occupancy rungs (__launch_bounds__(...,2) / amdgpu-max-vgpr 80/96: today 101-104 VGPRs = 8 waves/CU = 2 fronts/SIMD cannot hide 300-cycle HBM2 latency; <=96 VGPR = 3 fronts, <=80 = 4 - every variant oracle-gated, codegen changes). W31 P2P-ALLREDUCE (wt-p2p-ar, amd/p2p-ar): the flat P2P bypass of the RCCL ring - each die pushes its 20 KB slice to 3 peers (or pulls + local 4-way add), estimated 8-14 us vs the 69.5 us ring floor = ~-7.5 ms/cycle (+6 percent decode); desk answers gfx900/PLX P2P feasibility FIRST (hipDeviceCanAccessPeer, p2p sysfs, existing ggml machinery), then prototypes the probe kernel; numerics note: flat sum order differs from ring = same class as the E-090 RCCL sign-off, flagged for owner. (c) OWNER CONSTRAINT ADOPTED: no long prefill runs while levers are live - the battery stays 10k decode-only cells; the U1 deep rerun (v2, ignore_eos) stays DEFERRED until the lever queue drains; U1 in flight continues as-is. (d) The review independently reproduced the E-141 cycle math (MMVQ ~82 ms of 124 = the largest term) - cross-validation of the correction.
- E-143 2026-09-24 15:35 W27 Q40-PREFILL BENCH: BIT-EXACT 9/9 + -40..-44% AT DEPTH - U2 QUEUED. Oracle 9/9 cells BIT-EXACT (0 els differ) at depths 7168-199936, M=512 + tail 268 + padded used-boundary; staged KQ-X cells = DBG capture-region artifact (dst-exact overrides; stop rule refined to dst-is-truth). TIMING (basep -> v11p, med): d51200 550.9 -> 313.4 ms (-42.99 PERCENT, clean spreads); d102400 1409.2 -> 830.7 (-40.76); d199936 3061.7 -> 1687.7 (-43.97; mins -44.7). BETTER than the W24 model (-37.5) - the prefill tile is even more read-amp-bound than decode. U2 served window JUSTIFIED (prereq v11p >= 30 PERCENT met), QUEUED after the 5-window lever battery per the owner's no-long-prefill constraint (U2 needs 50k/100k prefill cells). Also: systemd-oomd DISABLED system-wide (it SIGKILLed the U1 cgroup at 14:26:55 and the user terminal - kernel OOM-killer remains); U1 ended PARTIAL by coordinator stop (10k full, 50k/100k prefill-valid, decodes EOS-void systematic, 150k/199k deferred to the v2 rerun which is now REQUIRED).
- E-140 2026-09-24 15:55 U1 DEPTH TABLE v1 (PARTIAL WINDOW) + THE QUEUE. (a) U1 closed partial (coordinator stop after the oomd event): u1_10k FULL 23.78 t/s @127.8 ms (law -2.2 PERCENT, accept 0.68, VOID PASS); u1_50k prefill-valid 29.41 t/s @51203; u1_100k prefill-valid 15.63 t/s @102395 (both near the W24 fresh-thermal model once soak is removed); decodes at 50k/100k EOS-VOID (systematic depth-dependent greedy EOS, 2/2); 150k/199k not run (killed 17 min into 150k prefill by oomd - systemd-oomd DISABLED afterward; it was also the terminal killer). (b) REQUIRED NEXT-RUN: /home/chris/run_u1_window_v2.sh (ignore_eos) regenerates ALL cells including deep decodes + acceptance-vs-depth points (the L4 rising-acceptance prediction stays UNTESTED until then) and the fa40 deep signature. SCHEDULED after the lever battery per the owner's no-long-prefill constraint. (c) W26 finalized (coordinator section in docs/amd-port/results/W26_u1_depth_table_2026-09-24.md).
- E-144 2026-09-24 16:10 W30/W31 NEGATIVES BANKED (merged 03f084029 + 8bfea3177) - REVIEW LEVERS CLOSED BY MEASUREMENT; BATTERY LAUNCHED. (a) W30: lb2 launch_bounds = codegen no-op (regs 84/77/112 + occupancy unchanged; (128,2) already satisfied); C4 GLU-fusion T=4 = ORACLE FAIL on iq4_xs (19449/69632 els, max_rel 0.114 - broad mismatch, NOT the W17 tile signature) and +21.8 PERCENT SLOWER on iq3_s (fewer CTAs x 2x work); max-vgpr 80/96 arms UNBUILDABLE (ROCm 6.2.0 LLVM 18 has no -mllvm -amdgpu-max-vgpr). Gates stay default-off; C4 closed as an exactness-preserving lever (a non-exact owner-sign-off variant is theoretically possible and NOT pursued). The oomd kill had also left 35 zero-byte .o objects in a desk build (cmake treated them as fresh) - repaired; watch for this after every oomd/crash event. (b) W31 P2P ALLREDUCE: DEAD EMPIRICALLY - all 12 ordered die-pairs canAccess=0 enable=0 (driver-level refusal; explains RCCL's own x12 enable failures); the host-staged-flat fallback WEDGES (zero completed iterations; probe watchdog fired). The reviewer's -7.5 ms/cycle lever does not exist on this stack. Receipt section 7 supersedes section 1. (c) NET: the external review's new levers are all closed by measurement; the standing queue (W23 env, clock floors, shape cache, chain-4, ub1024) remains the live lever set, plus U2 (q40-prefill served) and the v2 deep rerun. (d) BATTERY LAUNCHED on HEAD 8bfea3177 (5 windows, all 10k decode-only, ~8 h): results land in /home/chris/postu1_window_results.txt + per-window logs; promotions per the E-137 procedure on any paired WIN.
- E-145 2026-09-24 17:15 W23ENV EXACT-VOID OVERTURNED AS HARNESS FALSE-POSITIVE; NO-PROMOTE STANDS ON THE MERITS (W33, amd/v340-port-v2 d47828538). The w23env window ran --decode-only cells, which structurally never emit the determinism_guard line (guard_battery.py:895 skips it under that flag); the battery's adjudicator demanded it anyway -> det=None != PASS -> EXACT-VOID with a misleading "text_sha256 diverged" ratchet. The kernels are EXONERATED: all three gates route through the s2r-capable entry at the served shape (mmvq.cu:1713-1716; the hardcoded s2r=false at 1769 is the -sm legacy row only - no second E-117-class gap), the exact composition was host-proven bit-exact at served geometry T=1..4 (W7 A4), and the gates are integer-order-identical by construction (not even fp dust). Indirect witness: all four cells' decode counters IDENTICAL (draft_n=126, accepted=84, ratio 0.66667) - consistent with identical served text. NO-PROMOTE stands on the merits: +0.32 PERCENT mean is inside the window's own repeat spread (0.28 t/s) - statistically zero; gates stay unset. FIXES QUEUED (post-battery, script is live): run_post_u1_battery.sh w23env adjudicator must distinguish det-missing from det-fail (or drop exactness=True under --decode-only); the three gates' INFO lines -> WARN (the E-117 observability law was never applied to them). Observability gap noted: the served engagement witness for these gates is otherwise absent.
- E-146 2026-09-24 17:30 W34 FIXED-TEXT ACCEPTANCE INSTRUMENT BUILT (1c30abe71) + C4 ISA CHARACTERIZATION (d263dd9fb, double-derived + cross-verified) + battery ub1024 window in cells. (a) W34: the server ALREADY exposes per-position acceptance ("acc per pos = (...)" at -lv 5, server-context.cpp:619-633) and per-request draft_n/accepted - zero server code needed for the fixed-text instrument. Instrument built: frozen prompt-side token-id cascades (identical text across arms BY CONSTRUCTION - divergence cannot enter), p_i = P(accepted >= i) per position, scripts docs/amd-port/scripts/{ft_corpus.json,ft_cell.py,ft_score.py} + runner /home/chris/run_fixedtext_acceptance.sh (45-55 min, 4 short boots). GAP FOUND: target top-k logprobs are NOT populated on the spec path (TODO at server-context.cpp:4091) - spec'd (~3 lines LLAMA_TRACE-gated) if ever needed. (b) C4 FAIL characterized TWICE, independently, cross-verified: FMA-contraction divergence between template instantiations under -ffp-contract=fast - the fused instantiation contracts ALL accumulation chains to single-rounding v_fma (115 fmas) while the pair kernel double-rounds 7/8 chains (1 fma) -> 2-vs-3 roundings/step -> ulp drift on ~28% of outputs, max_rel 0.114 at near-cancellations; iq3_s both contract -> bit-exact. Contraction is instantiation-local (register pressure/schedule), NOT source-expressible; -ffp-contract=off would change served bit-streams. VERDICT DEAD: no type both exact AND faster (iq4_xs wrong AND 2.16x slower; iq3_s exact AND +21.8% slower). Gate stays default-off. (c) Battery: ub1024 window in R/A/A/R cells (17:17), hogs 7, watchdog positioning between cells as designed. (d) Housekeeping: E-140/E-145 receipts + W26/W32/W33/W34 all banked; next ledger E-147 = battery adjudications.
- E-147 2026-09-24 18:10 BATTERY FINAL: FIVE WINDOWS, ZERO PROMOTIONS - ALL QUEUED LEVERS MEASURED. (1) w23env: EXACT-VOID = harness false-positive (E-145); no-promote stands on merits (+0.32% inside 0.28 t/s repeat spread). (2) ub1024: SPLIT - decode +1.41 percent mean but prefill not positive in this run -> hold at 512/512 (the boot-4 contradiction did not replicate under battery conditions; the clean cooldown-gated re-window stays open). (3) w19 shape-cache: ARM-CRASH - NEW served signature ggml-backend-meta.cpp:832 GGML_ASSERT(src_ss[i].axis != SPLIT_AXIS_UNKNOWN), deterministic 2/2 arm cells with engagement=2 and controls PASS - the E-138 heal fixed the 1929 class but the re-split path has a SECOND unguarded hole (split-source axis assignment). VERDICT: shape-cache PARKED permanently (two crash paths for a ~3 ms lever; default-off, documented). (4) w22c1 clock floors: CTRL-FAIL - the clock-floor hook was refused (battery runs unprivileged; the window needs a root-capable run) - the lever is UNTESTED, retry warranted under sudo. (5) w28nmax chain-4: REJECTED DECISIVELY - paired -43.42/-43.55 percent, provenance clean, VOID PASS; decomposition: 167 draft tokens generated, 84 accepted - the IDENTICAL 84 that chain-3 accepted: position-4 acceptance ~0 PERCENT, mean_len unchanged, pure overhead (+33 percent draft, +25 percent verify span). The reviewer's 29.4 t/s math assumed acceptance holds; measured: the head's calibration ends at 3 tokens. (f) The draft_accept ACCEPT-MISSING label is an adjudicator parse gap for the chain-4 counters (server log carries the true numbers). Of-record UNCHANGED at 24.55. The lever cupboard for short decode is now: clock-floor retry (root), U2 prefill axis, v2 depth rerun, L7 owner-guarded. Nothing else survives its window.
- E-148 2026-09-24 18:25 UB1024 PROMOTED (the battery's one WIN, recovered by the analyst's server-log adjudication). Paired decode +0.36/+0.33 t/s (machine SPLIT was a harness artifact: --decode-only skips the prefill guard; the prefill co-metric lives in the server logs: +5.02/+4.14 PERCENT, matching the W24-family prediction). PROMOTION PROCEDURE EXECUTED: baseline decode ratcheted 24.55 -> 24.69 (the correct field this time: cells/decode_8k_10k/decode_per_second); --batch-size 1024 --ubatch-size 1024 applied to launch_tp3_200k.sh + run_combined_window.sh + run_postreboot_hardened.sh defaults + run_u1_window_v2.sh; syntax all OK; no rebuild needed (args only). OF-RECORD NOW: decode 24.69, prefill ~224 (ub1024 co-metric), accept 0.66-0.68, TP4+RCCL+fa40+ub1024. w22c1 clock floors: re-run queued under ROOT (the hook refused unprivileged execution - structural skip, lever UNMEASURED). NOTE for the record: three ratchet-anchor incidents now share one lesson - the guard and the ratchet must read the SAME field path (E-137 wrote the wrong path, E-148 fixed it; add a battery-side assertion that baseline-decode == the value the guard prints).
- E-149 2026-09-24 19:45 W22C1 CLOCK-FLOOR RETRY ADJUDICATED: KILL - LEVER CLOSED, NO BOOT-CHAIN CHANGE, NO RATCHET. Complete six-cell root re-run (the E-147(4)/E-148 queue): R1 24.66 / F1 24.34 / F2 24.13 / R2 24.49 (10k R-F-F-R block) + F200 23.90 / R200 24.04 (200k anti-decay pair, F then R). The floor arm lost ALL THREE pairs: F1-R1 -0.32 (-1.30%), F2-R2 -0.36 (-1.47%), F200-R200 -0.14 - and r200 held the warmer, later soak position and still won; paired 10k mean -0.34 (-1.38%) vs the >= +0.5-with-witness PROMOTE bar. The witness kills the mechanism, not just the number: power_dpm level=high does NOT pin under load - f1 pinned clean (<=991 duty 0.0%, die-3 mean 976->1240) and was STILL SLOWER (decode is not sclk-limited in the 1100-1300 MHz band); f2 leaked (991-band dips 9-27% under warm soak, act_mean 1192 below regular r2's 1302); f200 latched into the throttle band through the needle (gate VOID: die-3-of-record act_mean 1209, <=991 duty 22%, die2 775-band latch from ~100 s) - the W18 anti-decay claim refuted inside the arm built to prove it. R cells free-run the same bands with gates PASS: the dip exists but costs nothing measurable at this shape. Boot-decay deconfounded by pairing (R1->R2 -0.17, F1->F2 -0.21; r2 warmer-start still beat f1). HAZARD WITNESS: the 200k needle drives HBM to 90-93 C vs crit 95 on both arms (die1 93 C with floors set) - any future clock-forcing attempt must come from SMU/od8 level (not dpm_sclk) and must carry a memory-temp guard (recorded, NOT queued). CLOSED: C1 per-die clock floors on W22 kill criterion 1; set_clock_floor.sh stays OUT of the boot chain; of-record UNCHANGED decode 24.69. Residual: clocks never truly pinned, so the W10 t_d-compression discriminator never ran clean - the silicon/HBM-class question passes to C3 as the analysis-only closing post-mortem; C2 taskset unaffected; byte-rebalance already closed (E-125). Harness: r200's first floor-clear hit a teardown race (forced-high release delay) and the re-attempt ran clean - coordinator to add a post-teardown settle / one 90 s retry to the hook path; commit=unknown on all cells is a root git-read quirk (chain intact via identical binary/config hashes 0d5eb814b0653589/c2c5656128f0d531); acceptance 0.66667 identical across all six cells (numerics-neutral as designed). Receipt: docs/amd-port/results/W36_clock_floor_retry_2026-09-24.md.
- E-151 2026-09-24 21:05 q40-PREFILL MERGE LANDED + CI PASS + OBJECT-PURGE #3 (repair) + U2 LIVE. (a) MERGE: amd/q40-prefill (prefill tile arm GGML_CUDA_FATTN_TILE_Q40_PREFILL, W27 oracle 9/9 EXACT, -40..-44%/launch at depth) merged --no-ff as 53d0c97ac; build-hip rebuilt (binary e247afbbe8167ca3); CI decode-guard PASS on the merged tree with arm OFF: decode 24.71 vs baseline 24.69 (+0.1%), OVERALL PASS, provenance commit 13ccb8414 tree clean. Of-record unchanged 24.69. (b) PURGE #3: during the 20:21-20:22 session-bounce event (gdm/xrdp restart; U2 first fire killed mid-cell r1 at 20:20:52+; one coordinator exec interrupted) 8 git objects were left empty incl. the mid-write rev9 commit 5c0759fe and 1 index blob. Repair per playbook: branch reset 5c0759fe -> 13ccb8414 (provenance anchor: CI had verified that hash at 20:19), empty objects purged, index rebuilt via rm .git/index + read-tree HEAD, fsck clean; zero receipts/ledger lost; all desk branch refs intact. rev9 restated in full at c4dcc143f. Mitigation note: push_backups.sh after EVERY landing (followed). (c) U2 SHORT REFIRED 20:59 (campaign-u2-short2, port 8081, geometry 1024/1024 per W40 fix): r1 base cell MEASURED clean (VOID-GATE PASS, engagement prefill=0 direct=320 = correct base fingerprint). Verdict paths per +5% kill law; on PROMOTE -> U2 --long (50k/100k); on KILL -> arm stays default-off, next window = W39 NCCL env arms (NCCL_MAX_NCHANNELS=1, NCCL_SHM_USE_CUDA_MEMCPY=1, spec W39 s5). (d) W41 desk (wt-smallk, quantize_q8_1 re-quant dedup + TP4 copy deletion, W37 Opp-2) live through the incident in its own worktree.
- E-150 (late entry; events ~19:50-20:05, recorded after E-151 because both original append chains died on a gitignored-path git add) W38+W37 BANKED, TRANSPORT CLOSED, DENSE-ARCH CORRECTION, PUSH-BLOCKER DECISION. (a) W38 (amd/oneshot-ar da4fd7c69): one-shot allreduce DEAD on both substrates - peer access refused 0/12 ordered die pairs (W31) AND hipIpcGetMemHandle invalid-argument 0/12 (new ipc_open_probe, log W38_ipc_open_20260924_194437). Payload corrected: all 136 decode boundaries fp32, hidden 5120 (128 verify T=4 at 81,920 B + 2 catch-up T=4 + 6 draft T=1 at 20,480 B = 10.77 MB/cycle); 210.8 us/boundary = 69.5 us transport floor + ~141 us arrival skew; transport replacement closed permanently; live allreduce lever = skew compression only. (b) W37 (amd/roofline 7c318ee9e): MMVQ effective 54.7 GB/s on the verify pool (3.063 GB/die / 56.0 ms) = 1.3-2.2x off the schedule-family ceiling (70-120 GB/s, W5 A4), NOT 10x off peak; win inside the schedule family bounded -12..-30 ms/cycle; strict mul_mat_vec_q = 60.6 ms/cycle (49%), the dossier's ~80 ms / 65% bucket folds small kernels + host slices (dossier errata in W37). MODEL CORRECTION: arch is llama_model_qwen35 (src/models/qwen35.cpp), DENSE hybrid (16 full-attn + 48 gated-delta-net, no n_expert) - NOT MoE; all expert-routing premises in earlier receipts void. Ranked top-3: (1) whole-pool MMVQ schedule/occupancy lift -3..-14 ms -> 25.4-27.4 t/s; (2) small-kernel dedup (quantize_q8_1 3x re-quant 2.4->0.8 ms) + TP4 copy/convert deletion (4.3->~1.9 ms) -2.5..-3.5 ms -> ~25.5 t/s; (3) post-fa40 attention tile concurrency -2..-4 ms. All-kernel stack -8..-21 ms -> 26.0-30.0 t/s; 30 t/s needs op-1 top-of-band + skew compression. (c) Branch-push blocker (three ~578 MB census CSVs in the 346-commit unpushed range): DECISION DEFER - no history rewrite (provenance hash chain), push_backups.sh orphan snapshots remain THE content channel; desk branches may filter their own lineage before first push.
- E-152 2026-09-24 23:00 U2 SHORT x2: ARM EFFECT +18.7..+19.8 PCT PREFILL ON ALL FOUR PAIRS, PROMOTE LAW-BLOCKED BY THE <=991-DUTY GATE; GATE MISCALIBRATED AT 10K PREFILL; FRESH-BOOT RE-RUN QUEUED. (a) Data (base R vs arm A, 10k, 1024/1024, text sha IDENTICAL in every arm cell, engagement prefill=640 direct=320 exact): attempt-1 r1=179.07 a1=212.69 (+18.76 both-pass VOID-GATE) a2=212.25 r2=177.11 (pair-2 duty 22/27 pct VOID); attempt-3 (cool-boot 40C dies) r1=179.28 a1=214.15 (+19.44) a2=213.01 r2=179.34 (+18.75) with duty flapping 27/22/11/27 - attempt-3 VOIDed positions 1-2 while attempt-1 PASSED them on the same boot 30 min earlier. Base-cell spread 177.1-179.3 (1.2 pct), arm-cell spread 212.3-214.2 (0.9 pct), distributions fully separated; no decay at later positions (a2 213.01 vs a1 214.15 = noise). (b) MECHANISM FINDING: the void_gate <=991MHz-duty-<=20pct rule (built for the E-133 decode VOID class) does not calibrate for 10k prefill cells - die3 low-band duty swings 9-27 pct arm-INDEPENDENTLY, not monotone in boot temp (attempt 3 ran COOLER and gated WORSE), position, or recency; two full windows voided on the gate, zero on the arm (no SHA-VOID, no ENG-VOID anywhere). (c) VERDICT per the written law (E-117/W7 A4/E-133): do-NOT-promote on a VOID window stands; retries capped (one thermal retry taken, rev10); of-record prefill UNCHANGED (co-metric 217.71@2k era table; 10k base ~179). Status: PROMOTE-CANDIDATE BLOCKED AT THE GATE - the arm (GGML_CUDA_FATTN_TILE_Q40_PREFILL, merged 53d0c97ac) stays DEFAULT-OFF, binary CI-green, zero risk. (d) SANCTIONED PATH: one U2 SHORT re-run on the NEXT fresh boot (queued as the first cell of the postreboot queue when the owner re-enables it); alongside, GATE-CALIBRATION PROPOSAL for owner: per-window-class duty thresholds or prefill-t/s drift as the soak signal, since duty is arm-independent noise at prefill. (e) W39 NCCL env arms (NCCL_MAX_NCHANNELS=1, NCCL_SHM_USE_CUDA_MEMCPY=1) also queued for a window slot. (f) FLEET: subagent usage limit exhausted 22:4x, resets 2026-09-25 11:25 - campaign solo-mode until then; W41 smallk desk v2 died to the limit (no artifacts beyond CHECKIN); all W-receipts E-133..E-152 + rev1-10 banked and snapshot-pushed.
- E-153 2026-09-25 00:05 U2 THIRD WINDOW ON A TRUE COLD BOOT (postreboot queue, U2_ARMED consumed): arm effect REPLICATED A THIRD TIME, gate voided positions 2-4 again; GATE vs ARM question now closed as an evidence package. Full 6-pair dataset across warm/cool/cold boots: base 179.07/177.11/179.28/179.34/178.57/177.36 (spread 1.2 pct), arm 212.69/212.25/214.15/213.01/212.56/212.56 (spread 0.75 pct), every pair +18.7..+19.8 pct, text sha identical in all arm cells, engagement exact, zero decay. Cold-boot gate pattern: r1 PASS 9 pct then a1/a2/r2 VOID 22/22/27 - duty creeps WITHIN each window irrespective of boot state (cold boot gated WORSE than attempt-1's warm first cells), so the <=20 pct threshold is structurally unpassable for 4-cell prefill windows. VERDICT unchanged per law: no promotion; GGML_CUDA_FATTN_TILE_Q40_PREFILL stays default-off; of-record prefill unchanged. OWNER DECISION PACKAGE (formal): (1) recalibrate the E-133 duty rule for prefill window classes (evidence: 3 windows, 12 cells, duty arm-independent, in-window creep), or (2) accept the 6-pair documented evidence and promote the arm with the prefill ratchet (+~19 pct at 10k; U2 --long 50k/100k curve recommended before production reliance). Queue continues autonomously into the fresh-boot hb battery (campaign-postreboot, ~1.5 h). Subagent fleet resumes 11:25 (W41 re-dispatch + W37 Opp-1 MMVQ desk next).
- E-154 2026-09-25 00:45 FRESH-BOOT HB BATTERY: 4/4 PASS, UB1024 INDEPENDENTLY CONFIRMED, NIGHT QUEUE CLEAN. The postreboot queue (armed reboot 23:45) ran U2 (E-153) then the 4-cell hb paired geometry battery: 512/512 cells prefill 215.70/214.47 decode 24.35/24.22; 1024/1024 cells prefill 226.33/226.28 (+3.9 pct vs of-record 217.71) decode 25.06/25.06 (+1.5 pct vs 24.69) - the E-148 ub1024 promotion replicates on a cold boot (paired 1024-vs-512: prefill +4.9/+5.5 pct, decode +2.9/+3.5 pct). All cells OVERALL PASS, provenance commit 4df89533e tree_clean binary e247afbbe8167ca3 config c2c5656128f0d531. Of-record UNCHANGED (24.69/217.71 - the ratchet already priced ub1024 at E-148; fresh-boot cells are confirmation, not a new ratchet path). Boot healthy: watchdog quiet, zero deaths, churn spaced per E-134. Queue then exited clean; GPUs idle 00:30+. NEXT: W39 NCCL env-arms window (A1 NCCL_MAX_NCHANNELS=1, A2 NCCL_SHM_USE_CUDA_MEMCPY=1) firing on the idle night GPU.
- E-155 2026-09-25 04:25 W39 NCCL ENV ARMS: BOTH CLOSED - CHANNEL AXIS BRACKETED SHUT, SHM KNOB BROKEN. (a) A1 attempt 1 (NCCL_MAX_NCHANNELS=1 alone): RCCL 2.20.5 READ the env ("set by environment to 1") but formed the ring at Channel 00/02 anyway - MAX alone clamps; per the W39 engagement gate the cells are VOID (paired null +0.02 t/s is meaningless, arm never engaged). (b) A1b (NCCL_MIN_NCHANNELS=1 NCCL_MAX_NCHANNELS=1): ENGAGED - "Channel 00/01" in both arm cells, 00/02 in controls. Paired: na1-nr1 = 24.65-25.00 = -0.35, na2-nr2 = 24.52-24.57 = -0.05; verdict LOSS mean -0.20 t/s (-0.8 pct). Single channel = longer transport, costs more than any comm-SM-footprint relief. With E-119 (ch4 served loss), the channel axis is now BRACKETED AND CLOSED: 1 worse, 2 (default) optimal, 4 worse. E-119's record updates from "ch4 specifically bad" to "axis closed; default 2 optimal". (c) A2 (NCCL_SHM_USE_CUDA_MEMCPY=1): server init HANGS with the knob - 2/2 arm cells FAIL boot-timeout (stall in NCCL comm init, >800 s) while both controls boot clean (24.76/24.54 t/s). Knob UNUSABLE on RCCL 2.20.5; the wire term stays SM-store-optimal (W31 3.2 prior stands). One leftover hung server PID-killed post-window; dies 33C idle. (d) NET NIGHT RESULT: both W39 arms closed (one by measurement, one by breakage) - the allreduce wall's env-knob tier is now exhausted; the remaining allreduce levers are the skew rider on future kernel wins (W39 s3) only. The harness gained nccl_a1/nccl_a2 windows (R/A/A/R, INFO both arms, engagement fingerprints checked post-hoc). Fleet resets 11:25 - W41 smallk desk + W37 Opp-1 MMVQ schedule desk next.
- E-156 2026-09-25 05:50 W42 RPB1 RUNG FALSIFIED (oracle-first law catches a second C4-class arm); q4_K-only residual noted; q8_1 dedup window LIVE. (a) W42 desk delivered GGML_CUDA_MMVQ_RPB1 (rows_per_cuda_block 2->1 on the T=2-4 share body, register-diet hypothesis M2: iq3_s 83->~68 VGPRs = 7 CTAs/CU) claiming bit-exact by construction. Die-3 bench (campaign-w42-bench, receipt W42 s3 protocol executed verbatim): rpb1 is ORACLE BITEXACT ONLY ON q4_K (843.9 us vs share 870.7 = -3.1 pct, vs base 1107.5 = -23.8 pct); ORACLE FAIL on iq3_s T=4, q6_K, q3_K, iq3_s T=2 - systematic by_tok=last-tok by_rowpar signature (rel 1e-7..7.7e-3) = reduction-order change across the row-pair boundary. Where it failed it was also SLOWER (q6_K 6658 vs 6120 base, q3_K 999.7 vs 800.7). lb4s BITEXACT but catastrophic (q4_K 2656 vs 1107). Promoted share arms re-validated BITEXACT on every qtype. VERDICT: RPB1 falsified as a general rung - W37's +2 pct served gate decisively missed; q4_K-only residual (-0.26 ms/cycle, needs per-type gate) banked as available-below-bar; arm stays on amd/mmvq2 (9ed09aa4f), NOT merged, default-off. M2 partially confirmed (gain exactly where register pressure was worst); M1 runway + the family ceiling stand. The in-family schedule ladder has no remaining cheap rungs - the 70-120 GB/s climb needs the past-family redesign W37 scoped (LDS-y route killed twice, W7). (b) Artifact: /home/chris/w42_bench_full.log (144 lines, all sessions + oracle verdicts). (c) IN FLIGHT: q8_1 dedup arm (GGML_CUDA_Q81_ACT_CACHE=1, merged eda3141b9, CI PASS binary ac27d7c3e8643f25) paired window campaign-q81-win (R/A/A/R 10k decode-only, arm env in a-cells only; engagement WARN line checked post-hoc). W41 gates: >= +0.3 pct t/s AND >=1.8x fewer quantize launches (timeline boot after the window).
- E-157 2026-09-25 06:30 W37 OPPORTUNITY-2 CLOSED (both branches); q8_1 arm no-promote at the gate. (a) q81 paired window (campaign-q81-win, merged binary ac27d7c3, arm env in a-cells, engagement line verified present in arm server logs only): qr1 24.52 / qa1 24.48 / qa2 24.58 / qr2 24.40 -> SPLIT mean +0.07 t/s (+0.29 pct) - BELOW the W41 +0.3 pct kill gate AND not a paired WIN (one negative pair). VERDICT: no promotion; GGML_CUDA_Q81_ACT_CACHE stays default-off in tree (merged, CI-green, zero risk). The LLAMA_LAUNCH_TIMELINE measurement boot (campaign-q81-tl) ran but the instrument reports per-ubatch meta-node classes (nodes/replays/bounds/comm/sched cpy counters), NOT per-kernel quantize counts - the >=1.8x launch proof is unobtainable from it; the served delta closes the arm regardless. (b) CLASS CLOSURE: Opp-2 dedup branch measured at noise level (the 2.2x re-quant dedup's served value < 0.3 pct - W37's 2.4 ms quantize term overstates the dedupable slice); Opp-2 copies branch closed by the W41 census (GDN state glue + KV set_rows + MTP concat are load-bearing; no convert nodes). (c) W37 KERNEL LADDER STATUS: Opp-1 RPB1 falsified (E-156), Opp-2 closed (this entry) - remaining: Opp-3 post-fa40 attention tile concurrency (-2..-4 ms/cycle, grows with depth; W17 latency-bound evidence). Decode of-record 24.69 unchanged; plateau ~24.4-25.1 across all fresh-boot cells tonight.
- E-158 2026-09-25 07:25 DEATH #6 (mid-persistent-cell-3), RECOVERY CLEAN, PERSISTENT BASELINE PARTIAL (2 cells), PROTOCOL RULE REVISED. (a) The owner-approved persistent-server battery (protocol persist-1: one boot, N guard cells) ran c1 24.88 / c2 24.95 (both OVERALL PASS, provenance 6408d20fc/ac27d7c3) and died at c3's guard launch ~06:59:45 - HARD DEATH #6, machine rebooted 06:59:50. Post-death protocol clean: fsck CLEAN (no object purge), repo intact at rev15, boot queue self-fired (U2 gate skipped, hb cells running). (b) MECHANISM NOTE: the persistent protocol COMPRESSES churn server-side (2 guard cycles + request buffers in 8 min on one KV pool); the death class therefore tracks in-server allocation churn too, not just boot/teardown cycles. RULE REVISED: protocol persist-2 = max 2 cells per server boot (still halves boot count vs legacy, bounds per-server churn); every persistent run starts with the lock check per cell (already in the script). (c) BASELINE so far: persistent-of-record candidates 24.88/24.95 vs fresh-boot-cell band 24.4-25.1 - the protocol measures clean; c3/c4 re-run after the hb queue drains on this boot (2-cell cap). (d) Both morning desks (Opp-3 attn, L7 recipe) died in the reboot (~06:59); re-dispatched with resumed-state briefs; wt-attn3 CHECKIN last write 06:58 (partial line - death artifact).
- E-159 2026-09-25 09:55 PREFILL ARM PROMOTED (OWNER GO); L7 TIER A FAIL -> DEMOTED TO CONS (regex ladder bug found + fixed). (a) PREFILL ARM: owner approved the phase-split promotion ("U2 prefill arm split is approved"). GGML_CUDA_FATTN_TILE_Q40_PREFILL=1 flipped into ALL serving configs (launch_tp3_200k.sh + tierA derivative + battery + persist + CI wrappers); binary unchanged (arm was already merged env-gated, CI-green). 200k invariance cell RUNNING (campaign-prefill-200k: full battery, arm ON, needle+determinism at depth = the one unverified axis). Of-record prefill number updates only after this cell + U2 --long later. (b) L7 TIER A ABBA GATE: baseline 2/2 PASS (24.93/25.31 t/s, accept 0.661/0.693, needle 3/3, text sha IDENTICAL e478fdf3 across both boots); Tier A 2/2 FAIL - needle recall 1/3 and 2/3 (bar: equal 3/3) and B1 nondeterministic IN-BOOT at temp 0 (first-ever determinism failure in campaign history) - despite decode 25.76/27.34 (the accept-rate variance 0.656-0.778 across boots inflates cross-boot t/s comparisons; the quality gates are the signal). DEMOTED per the pre-registered fallback: output-head move (P1) is the prime suspect and is DROPPED. (c) TIER A-CONS built: head stays q6_K; 92 tensors (46 q5_K + 46 GDN q4_K) -> IQ4_XS; artifact 12,145,716,032 B (-107 MB vs source), sha16 18da578b50293e78; census-verified exact (only the three intended class changes). Cons gate = 2 B-boots vs today's fresh A pair (halved churn per death throttle; protocol note filed). (d) HARNESS/TOOL LESSONS BANKED: (1) llama-quant --tensor-type-file patterns are REGEX, first-match-wins, UNANCHORED - "blk.1.*" matches "blk.10.*" and scrambles two-digit layers (14 tensors -> Q6_K, 48 -> F32 in the first cons build); ALL ladder entries must be anchored escaped patterns; census-verify every artifact. (2) llama-quant threw imatrix-required on pure-copy tensors; fixed in llama-quant.cpp (requirement now gated on target != current). (3) run_tp3_guards.sh was pre-E-112 (no attribution) - fixed; the desk3-era ABBA wrapper needed three fixes total to run again.
- E-160 2026-09-25 10:40 DEATH #8 - STORM DECLARED, GPU CHURN STOPPED, PURGE #4 REPAIRED. (a) DEATH #8 at ~09:52:40, 55 min after death #7: the 200k prefill-invariance battery boot (09:48) died mid-battery. THREE hard deaths in 3h53m (06:59, 08:47, 09:52), each within minutes-to-an-hour of GPU server churn. This rate is hardware-trajectory territory, not the old boot-churn class: yesterday the same profile ran 25+ boots clean on one charge of the boot. (b) STORM RESPONSE: ALL GPU CHURN STOPPED. Pending GPU items queue behind a calm-machine period of owner's choosing: the prefill-arm 200k invariance cell (killed by death #8, re-run pending), the Tier A-cons acceptance gate (2 boots), the W43 --conc bench, the persist-2 control. Dies idle 36C, no servers running. (c) PURGE #4 at death #8: 10 empty objects incl. the amd/wide-s2r-era push target; ALL 8 campaign branch tips verified intact (fsck clean after purging empties); ABBA receipts banked; backup current at 093fea4e8. The stale wt-wides2r worktree pins the wide-s2r ref (harmless, left in place). (d) This boot's postreboot queue CORRECTLY REFUSED (binary predates the src/llama-quant.cpp fix commit - provenance fail-closed; llama-server needs a rebuild before any further serving anyway). (e) OWNER ATTENTION: the machine itself is now the bottleneck - recommend physical/thermal inspection (the W22 era measured HBM at 93C vs 95C crit under clock floors; today's deaths are not temperature-correlated at idle-cool starts, which points at power delivery or HBM degradation under burst load instead). The campaign stays zero-GPU until the box is inspected or a long cool-down elapses.
- E-161 2026-09-25 11:10 PREFILL ARM PROMOTION VERIFIED - 200k INVARIANCE GATE PASS. campaign-prefill-200k2 (storm canary, one boot on the 09:52 rebuild binary bc0a5a83a5736669): OVERALL PASS - prefill/decode/canary/determinism/needle ALL PASS. Decode 25.42 t/s (+2.96 pct vs of-record 24.69), determinism text IDENTICAL with the arm ON (the depth-invariance axis closed - served text byte-matches the of-record sha with PREFILL engaged), needle 3/3, accept 0.6930 (above baseline). The machine survived the full boot+battery (storm canary: 1 clean cycle post-death-#8). PROMOTION COMPLETE: the prefill arm is of-record serving config (BASEENV in all four runners); of-record decode updates 24.69 -> the guard's own field on the next ratchet cycle (the invariance boot measured 25.42 under the promoted config; conservative ratchet after a second confirming cell per the paired law). NEXT: Tier A-cons gate (2 boots) per the one-item-per-boot throttle.
- E-162 2026-09-25 11:35 DEATH FORENSICS (owner-requested log audit): ALL FOUR DEATHS ARE INSTANT HARD CRASHES WITH ZERO OS TRACE. (a) journal tails of boots -1/-2/-3 end mid-heartbeat (last lines are the zcode 30 s idle heartbeat); kernel rings end on OLD messages - no panic, no MCE, no thermal trip, no amdgpu fault at any death. last -x marks every session end "crash". (b) The svm hog counter does NOT predict death: boot -3 logged hog 11 at 08:30 and survived 17 min; boot -4 logged hog 19 at 01:51 and lived 5 h; boot -1 died with ZERO warnings; boot -2 logged hog 5 at 09:48 and died 4 min later. The hog is a churn SYMPTOM, not the tripwire. (c) EDAC: ie31200 "No ECC support" - memory errors are invisible by design on this board. No /dev/watchdog configured. (d) HISTORY: last -x holds 41 crash-terminated sessions with wildly varying uptimes (22 min .. 3+ days, incl. a doubled 03:59 pair) - this box has ALWAYS crashed under campaign duty; today's density (4 in 5 h) tracked today's record churn density (failed-fast ABBA attempts + 3 gate suites = the highest boot/teardown + guard-cycle rate of the campaign). Dies are power-capped at 110 W each (440 W GPU total). (e) CONCLUSION: not a new software regression and (by instant-reset signature) likely below-OS: prime suspects are power delivery under 4-GPU burst transients or non-ECC memory, both invisible to logs. MITIGATIONS: (short) strict churn rationing - one boot per >= 10 min, <= 6 guard cycles/hour; (medium) svm-watchdog v2 that PID-kills serving servers at hog >= 3 (converts some deaths to pauses); (long) ROCm 6.3+/kernel upgrade for the KFD svm class, memtest86+ overnight, PSU inspection. GPU work stays STOPPED pending owner's call on throttled-resume vs hardware checks.
- E-163 2026-09-25 13:50 THROTTLED REGIME VALIDATED - ZERO DEATHS THROUGH CHURN; WATCHDOG v2 LIVE SAVES; hb BATTERY COMPLETE; cons gate fired. (a) The post-storm regime (watchdog v2 kill-at-hog-4-sustained + lock janitor + one-item-per-boot throttle) ran the full hb battery on the 11:15 boot: p0a 24.34 / p1a 24.77 / p0b 24.53 / p1b 24.92 - ALL PASS, 1024/1024 beats 512/512 in-run by +0.9-1.2 pct (ub1024 confirmed a third time), machine UP 2.5+ h through multiple server cycles. (b) WATCHDOG v2 LIVE SAVES at 13:05:06 and 13:06:06 (hog=5 climbing): serving PIDs killed, cells lost, MACHINE SURVIVED - the exact scenario that produced deaths #6-#8. Two interactions fixed en route: root-owned lock files now chowned to chris on watchdog claim; stale locks (>30 min, no live server) auto-cleared by a janitor clause; a first cons attempt crashed on an unbound $tag in the hardened runner's boot-timeout path (fixed to $arm) plus a stale p1a lock that idled a cell 20 min (janitor now covers). (c) IN FLIGHT: Tier A-cons quality gate (campaign-l7-cons: 2 B-boots vs today's fresh A pair - pass bars: needle 3/3, in-boot determinism, acceptance within 0.02). (d) QUEUED after cons: W43 --conc bench, persist-2 control, prefill ratchet decision (invariance cell already PASS at decode 25.42 / prefill 244.18, E-161).
- E-164 2026-09-25 14:15 L7 IQ4_XS FAMILY CLOSED (banked null) - cons variant reproduced the Tier A failure EXACTLY. (a) Tier A-cons gate (campaign-l7-cons2, 2 boots vs today's fresh A pair): c3_B 24.56 t/s / accept 0.62963 / needle 2/3; c4_B 24.57 / 0.62963 / 2/3 - both FAIL, IDENTICALLY (same accept to 5 decimals, same text sha 9ec0b931 = deterministic reproduction of the quality hit). vs baseline A pair 24.93/25.31, accept 0.661/0.693, needle 3/3, sha e478fdf3. The cons recipe (head preserved, 92 GDN/q5_K tensors -> IQ4_XS) delivers NO served speedup AND breaches both quality bars (acceptance -0.031..-0.064, double the 0.02 bar; needle 2/3 vs 3/3). (b) CONCLUSION: the W44 speed model (bytes x rate) did not survive served contact - iq4_xs requant-down of this imatrix-tuned ladder costs quality at no measured speed benefit on gfx900 MMVQ; the desk's "~50 GB/s q4_0 rate has no receipt" correction cuts both ways (the 75 GB/s iq4_xs rate is equally unproven served). Family CLOSED per the pre-registered second-miss rule; artifacts kept (tierA sha 1e0f8250, tierAcons sha 18da578b) for any future imatrix-regen path; W44 stands as the recipe map. (c) The prefill-arm promotion (E-159/E-161) is UNAFFECTED - that arm is kernel-level, bit-verified. (d) NEXT: W43 --conc bench (Opp-3 falsifier), persist-2 control; then the day's queue is complete except owner-blocked items (U2 --long, U1 v2).
- E-165 2026-09-25 14:35 OPP-3 CLOSED (falsifier fired, negative both axes) - W37 KERNEL LADDER FULLY MEASURED OUT. W43 --conc bench (die HIP-dev-2, depths 7168/10240/65536, oracle-controlled per W43 s-adjudication): v11 control BIT-EXACT in-run; fill arms exceed even the DUST-PASS bar (max_rel 4.617e-03 vs 1.3e-03 limit - combine-order change visible) AND are SLOWER at every split at every depth: fill56 +4.9 pct (485->509 us d7168), +7.5 pct (4133->4442 us d10240), +10.8 pct (635->703 us d10240-2); fill84 +8.4; fillmax +30. W43's residency-saturation model CONFIRMED (126/128 CTAs already in flight - splitting only adds combine overhead); W37's -25..-45 pct/launch model FALSIFIED. VERDICT: Opp-3 CLOSED measured-negative; the shipped pb-scan is optimal for this kernel on gfx900. THE W37 KERNEL LADDER IS NOW FULLY CLOSED AT THE CHEAP TIER: Opp-1 RPB1 falsified (E-156), Opp-2 dedup/copies closed (E-157), Opp-3 falsified (this entry) - the 26-30 t/s kernel-stack projection from W37 is dead at the cheap tier; remaining decode paths are the past-family redesign (W5 LDS-y, killed twice) or owner-blocked items (U2 --long, L7 non-iq4_xs formats, U1 v2). OF-RECORD with the promoted prefill arm: decode 25.42 / prefill 244.18 (E-161 invariance boot). (b) IN FLIGHT: persist-2 control (campaign-hb/persist, 2 cells/server).
- E-166 2026-09-25 14:50 OF-RECORD RATCHETED TO THE PROMOTED CONFIG: decode 24.69 -> 25.33, prefill 217.71 -> 244.18 (+2.6 pct / +12.2 pct). Evidence chain: owner-approved promotion (E-159) + three independent post-promotion measurements (invariance boot 25.42/244.18 E-161; persist-2 cells 25.33/25.32 E-163-era) - conservative decode ratchet uses the LOWEST post-promotion measurement (25.33). Ratchet mechanics: baseline decode_per_second (guard-read field) and prefill_2k prompt_per_second (the field guard_battery.py:449 compares; prompt_tps added alongside is display-only - E-137 lesson re-learned and this time pre-checked). Both artifacts (tierA/tierAcons ggufs, sha16 1e0f8250/18da578b) remain on disk parked. CAMPAIGN STATE: the cheap-lever era is complete - GPU queue EMPTY. Remaining agenda is owner-gated: U2 --long depth curve (long run), U1 v2 deep rerun (owner-deferred), L7 non-iq4_xs formats (needs f16 source + imatrix regen), ROCm/kernel upgrade for the death class, hardware inspection. Machine healthy under the throttled regime (watchdog v2 live, 5h+ uptime through churn).
