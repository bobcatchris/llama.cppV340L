...
**Repo:** llama.cpp, branch `amd/v340-port-v2`, HEAD `53a2b70cd` era, synced to ledger
E-155 (2026-09-25 ~04:30). NOT upstream
...
promotion, E-148); the owner believes ~30 t/s should be reachable. W37 roofline
accounting (E-150) prices the honest path: strict MMVQ pool 60.6 ms/cycle (49%,
54.7 GB/s effective = 1.3-2.2x the schedule-family ceiling); the all-kernel stack
lands 26.0-30.0 t/s, and 30 needs the top of the MMVQ schedule band PLUS skew
compression. The cycle budget (E-141 + W37 corrections): ~18-25% of each speculative
cycle is inter-GPU allreduce latency (136 boundaries/cycle over PCIe - structurally
hard to remove; transport replacement DEAD permanently, W38/E-150; the env-knob tier
exhausted, E-155); the largest single term is the MMVQ weight-stream pool
(latency-starved). ASCII throughout (project rule). Claims cite: ledger entry
...
Qwen3.8-27B-ASCII-P1M.gguf: 27B params, 65 layers, hidden 5120, DENSE HYBRID (W37/E-150
model correction - NOT MoE: arch llama_model_qwen35, src/models/qwen35.cpp; no
FFN_*_EXPS tensor, no routing/permute ops in any census; 16 of 65 layers are full
attention, the other 48 are gated-delta-net recurrent layers, full_attn_interval 4),
with a 1-layer MTP draft block (nextn_predict_layers=1) for speculative decoding. Served at
...
|---|---|---|
| decode | **24.69 t/s** | 8k-10k context, temp 0, full stack (E-148 ratchet, ub1024; 24.55 = the pre-ub1024 E-137 stamp); merged-tree CI PASS 24.71 (E-151), fresh-boot hb battery 25.06 @1024/1024 (E-154, confirmation cells) |
...
| prefill | **217.71 of-record (gate)**; E-148 priced ub1024 at the decode ratchet only - the ~224 co-metric is informational; 10k base cells 177-179 t/s (U2 controls, E-152/E-153); fresh-boot 1024/1024 226.3 (E-154); 82.17 @8k; 170.35 @10k was pre-ub1024 | geometry 1024/1024 canonical |
| prefill, U2 arm (default-off) | **+18.7..+19.8% @10k on ALL SIX pairs across three boots** (arm 212.3-214.2 vs base 177.1-179.3; text sha identical; zero arm-cell failures) - promotion LAW-BLOCKED by the <=991-duty gate, owner decision pending | E-152/E-153 |
| prefill at depth | 170.35 @10k; 29.41 @51k | U1 cells, measured 09-24 (pre-ub1024 binary) |
| MTP acceptance | 0.66667, stable across all measured cells (clock-floor, U2, hb windows); of-band 0.66-0.68 | temp 0; mean accepted len ~3.04 of 3-token chain |
...
|---|---|---|---|
| **136 allreduce boundaries** (TP4 per-layer sync) | **~28.7-31.2 per CYCLE (census variant 22.7-25.6, W37 row 2)** | **18-25% of cycle** | W8 probe: per-die per-boundary medians 128.8/151.9/191.6/210.8 us; wall = slowest die: 136 x 210.8 us = 28.7 ms. SCALE NOTE (E-141): the wall is per speculative CYCLE (~124 ms = 3.04 tokens / 24.69 t/s), not per token |
| MMVQ weight matvecs, strict mul_mat_vec_q all passes (3.08 GB/die/verify) | **60.6 of the cycle (49.2%)** | **THE LARGEST SINGLE TERM** | W37 census accounting (E-150): verify 56.0 + draft 3.4 + catch-up 1.2; the earlier ~80-85 ms / 65% bucket folds in small kernels+copies (~10.5) and host slices (~10.7) - same verdicts on either reading |
| attention (16 KV layers, fa40 direct arm) | ~10.7 per cycle (verify tile 7.6 + draft/catch-up/combine) | ~8.7% | W37 row 3 (census 16.1 pre-fa40 minus the W17 -37.5% verify-tile cut; band 9.5-11.5); the W17 -37.5%/launch is the arm's relative win |
| KV pool conversion (f16 pool -> tiles) | 0 | 0% | eliminated by fa40 (need_f16=false); was 3.98 us/1k tokens/launch |
| host slices (post-verify drain/sample/accept + draft-loop host) | ~10.7 per cycle | ~8.7% | W37 rows 5+6, census MEASURED (the W13 "<1.4 ms" was the per-decode-call slice only) |

The structural statement (CORRECTED E-141, refined W37/E-150 - the earlier "sync is
70-75%, compute is nearly free" framing divided a per-cycle wall by a per-token time):
**the strict MMVQ pool is 60.6 ms/cycle (49%, 54.7 GB/s effective on the verify pool =
1.3-2.2x off the 70-120 GB/s schedule-family ceiling - latency-starved, not
DRAM-saturated); the allreduce wall is 18-25% (136 boundaries/cycle, structural); small
kernels+copies ~10.5 ms, host slices ~10.7 ms, attention ~10.7 ms, the rest microgaps
(W37 full-cycle table).** Tensor
parallelism requires 2 allreduces per layer x 65 layers + 2 catch-up + 6 draft-chain
boundaries = 136/cycle; the count is STRUCTURAL (W8 section 3; W39 section 2.4
re-verified 2 column-split matmuls per trunk block in qwen35.cpp). CONSEQUENCE: the MMVQ
access-pattern work (section 4.4, W37 Opportunities) targets the LARGEST term; the sync
wall's env-knob tier is EXHAUSTED (E-155) and its direct skew actuator is KILLED
(clock floors, E-149) - the surviving allreduce lever is the skew rider on kernel wins
(W-coupling law: skew = 0.078 x per-boundary work, W39 section 3). PP (serial staging) removes the sync but
...
Per-boundary cost = **ring transport floor** + **arrival skew**. Payload of record
(corrected W38/E-150): ALL 136 decode boundaries are fp32, hidden 5120 - 128 verify
T=4 + 2 catch-up at 81,920 B (94-96%) + 6 draft T=1 at 20,480 B = 10.77 MB boundary
payload/cycle; the boundary is latency/protocol/skew-bound, never bandwidth-bound.
...
| arrival skew T(die) | **59 us (die 3) to 141 us (die 1)** | W10: multiplicative device-side scaling, t_d = 0.893/0.846/0.964/1.000 per die (to 0.1%); W39 correction: these per-die medians are WAIT REDISTRIBUTION (die 1's 141 us contains 90.8 us spinning for die 3, which OVERLAPS die 3's compute) - the honest compressible skew is the per-die RATE spread, 45.2 us/boundary = 3.0-6.1 ms/cycle (+2.4-4.8%) |
| transport itself | flat x1.07 isolated | W9 - NOT the variable |
| RCCL channel count | **AXIS BRACKETED CLOSED: 1 channel -0.20 t/s served, 2 (default) optimal, 4 -12.6% served** | E-119/E-155 |
...
wait; it is NOT load imbalance (the dense qwen35 shards every tensor in exact quarters -
shards divide by 4 with built-in per-layer remainder rotation, W39 section 2.1; the
E-125 byte-rebalance arm lost served). The direct actuator is DEAD: the W36/E-149
clock-floor root re-run KILLED the lever (floors lost all three pairs -0.32/-0.36/-0.14
t/s; dpm level=high does NOT survive load; decode is not sclk-limited in the 1100-1300
MHz band; HBM 90-93 C vs crit 95 hazard witness). The 3.0-6.1 ms/cycle prize stands
with no remaining direct actuator - it now rides as the W-coupling rider on kernel wins
(skew = 0.078 x per-boundary work, W39 section 3), plus the queued W22-C2 taskset item
(0-1% class).
...
### 4.4 The weight matvecs - MMVQ (ggml/src/ggml-cuda/mmvq.cu) - 60.6 ms/cycle strict (W37 census; the verify pass alone is 56.0)

One templated kernel serves ALL decode matvecs for all weight types (3.063 GB/die per
verify pass, W37; 3.08 GB/die/round of-record:
...
W37 roofline accounting (E-150, zero-GPU) put the wall in scale: the pool runs at
**54.7 GB/s effective on the verify pool (3.063 GB / 56.0 ms; 54.8 over all 3.42 GB
must-stream = 60.6 ms) - 6.0x off the measured 327 GB/s consume wall but only
1.3-2.2x off the schedule-family (cfull) ceiling**, i.e. a launch/occupancy/schedule
problem bounded INSIDE the family at -12.2..-30.5 ms/cycle. Ranked top-3 kernel
opportunities (each with a falsifiable prediction gate): (1) whole-pool schedule/
occupancy lift into the cfull band, -3..-14 ms -> 25.4-27.4 t/s (LDS-y staging /
register diet to >= 8 CTAs/CU); (2) small-kernel + copies class (quantize_q8_1 3x
re-quant dedup 2.4 -> 0.8 ms + TP4 copy/convert deletion), -2.5..-3.5 ms; (3) fa40
attention-tile concurrency at serve depth, -2..-4 ms. All-kernel stack: 26.0-30.0
t/s; 30 t/s needs Opp-1 top-of-band PLUS skew compression. W41 (wt-smallk) owns
Opp-2's first half.

### 4.5 Attention - fattn-tile + the fa40 arm (ggml/src/ggml-cuda/fattn-tile.cuh) - ~10.7 ms/cycle post-fa40 (W37; the -37.5%/launch win is relative to the f16-pool arm)
...
NUMA), transport tuning (W9 flat x1.07), channel count (ch4 -12.6% served; ch1 engaged
-0.20 t/s - the axis is bracketed shut, E-119/E-155), the whole W9 27-config env sweep
(PROTO/ALGO/NTHREADS/MSCCL/P2P-off neutral; do not re-roll), count reduction (forced
structure), same-tensor clustering (no legal candidate), NCCL_SHM_USE_CUDA_MEMCPY
(server init HANGS 2/2 on RCCL 2.20.5 - unusable, E-155). Transport REPLACEMENT is dead
permanently: flat P2P bypass 12/12 canAccess=0 + host-staged fallback WEDGES
(W31/E-144), one-shot AR loses its substrate AND hipIpcGetMemHandle returns
invalid-argument 0/12 pairs (W38/E-150 - "P2P-via-IPC NOT available"; even if the
substrate existed, one-shot buys only +3.2-4.5% with skew untouched, +7.6% absolute
ceiling). What remains: compressing the skew - and its direct actuator (per-die dpm
clock floors) is KILLED (E-149/W36: floors don't survive load, arm lost all pairs,
HBM 93 C hazard). The surviving allreduce lever is the W-coupling skew rider on future
kernel wins (skew = 0.078 x per-boundary work; every W37 compute cut compresses the
skew proportionally, W39 section 3). Eliminating boundaries structurally is CLOSED: PP
NO-GO x2 by derivation (E-141/W29).
...
run_u1_window_v2.sh; args-only, no rebuild. Of-record now: decode 24.69, prefill gate
217.71 (the co-metric ~224 is informational), accept 0.66667, TP4+RCCL+fa40+ub1024.

INDEPENDENT CONFIRMATION (E-154, 09-25 00:45): the fresh-boot hb paired geometry
battery ran 4/4 OVERALL PASS on a cold boot - 512/512 cells prefill 215.70/214.47,
decode 24.35/24.22; 1024/1024 cells prefill 226.33/226.28, decode 25.06/25.06 (paired
1024-vs-512: prefill +4.9/+5.5%, decode +2.9/+3.5%). The E-148 promotion replicates;
of-record UNCHANGED (the ratchet already priced ub1024 - confirmation, not a new
ratchet path). Provenance commit 4df89533e, binary e247afbbe8167ca3.

### 5.5 The U2 q40-prefill served windows (E-152/E-153, 09-24 21:xx - 09-25 00:05)

The merged q40-PREFILL arm (GGML_CUDA_FATTN_TILE_Q40_PREFILL, merge 53d0c97ac,
default-off, CI PASS decode 24.71 vs 24.69 with arm OFF, E-151) ran three U2 SHORT
windows across three boot states (warm, cool 40 C, true cold). W40 preflight had
declared the harness READY after fixing one blocking defect (stale 512/512 geometry
default -> 1024/1024, in place, 6 hunks) and verifying the oracle carries over
(byte-identical prefill region vs the W27 bench tip). The arm effect:

```
base R (6 cells): 179.07 / 177.11 / 179.28 / 179.34 / 178.57 / 177.36 t/s  (spread 1.2%)
arm  A (6 cells): 212.69 / 212.25 / 214.15 / 213.01 / 212.56 / 212.56 t/s  (spread 0.75%)
every pair +18.7..+19.8% at 10k prefill; text sha IDENTICAL in every arm cell
(temperature 0); engagement prefill=640 direct=320 exact; zero decay at later positions
```

Distributions fully separated, replicated on all three boots, ZERO gate failures
attributable to the arm itself (no SHA-VOID, no ENG-VOID anywhere). VERDICT per the
written law: PROMOTE-CANDIDATE BLOCKED AT THE GATE - the battery's <=991 MHz-duty
void rule (built for the E-133 decode VOID class) does not calibrate for 10k prefill
cells: die-3 duty flaps 9-27% arm-INDEPENDENTLY, creeps WITHIN each window irrespective
of boot state (the cold boot gated WORSE than the warm attempt-1), and is not monotone
in position, recency, or temperature. Two full windows voided on the gate. The arm
stays DEFAULT-OFF (binary CI-green, zero risk). OWNER DECISION PACKAGE (formal,
E-153): (1) recalibrate the duty rule for prefill window classes (evidence: 3 windows,
12 cells, duty arm-independent, in-window creep), or (2) accept the 6-pair documented
evidence and promote with the prefill ratchet (+~19% at 10k; the U2 --long 50k/100k
curve recommended before production reliance).

---

## PART 6 - THE WALLS (each: the barrier, the attempts, the verdict)

### WALL 1 - The inter-GPU synchronization wall (structural; env/skew arms exhausted)

The 136 per-layer TP4 allreduces cost 28.7-31.2 ms per speculative CYCLE (~124 ms;
census variant 22.7-25.6 ms, W37) -
18-25% of the cycle (E-141 correction; the earlier per-token "70-75%" framing was a
scale error). Proven sealed: count reduction (2 cuts/layer forced by the sharding table + PARTIAL cut rule;
alternatives multiply weight streaming; W39 re-verified the count in code) and
same-tensor clustering (boundaries are
data-dependent, one tensor each, separated by nonlinear consumers - norm(partial +
residual) != norm(sum), fusion forbidden; enqueue already
stream-pipelined). Proven dead: the channel axis BRACKETED SHUT (ch4 -12.6% served
E-119; ch1 engaged and LOST -0.20 t/s E-155 - default 2 optimal), the NCCL env-knob
tier (W9 27-config sweep neutral; NCCL_SHM_USE_CUDA_MEMCPY hangs server init 2/2,
unusable on RCCL 2.20.5, E-155), tensor-split rebalance
(paired -1.92/-2.33; placement imbalance is structurally impossible - shards divide
by 4 exactly with built-in rotation, W39), host pinning (0-1% class), transport tuning
(W9 flat), transport replacement (P2P 12/12 canAccess=0 + host-staged WEDGE, W31/
E-144; one-shot AR + IPC handles 0/12 invalid-argument, W38/E-150 - DEAD PERMANENTLY),
and the direct skew actuator: W22-C1 per-die clock floors KILLED by the six-cell root
re-run (E-149/W36 - the floor arm lost all three pairs -0.32/-0.36/-0.14 t/s; dpm
level=high does NOT survive load; decode is not sclk-limited in the 1100-1300 MHz
band; the 200k floored cell latched into the throttle band anyway and drove HBM to
90-93 C vs crit 95 - recorded hazard, not queued). CLOSED: PP NO-GO x2 by derivation
(E-141/W29) - PP4
serializes the concurrent compute: cycle = 4C + ~25 ms = ~350-365 ms = 8-9 t/s
...
monotonically worse. The surviving allreduce lever: the W-coupling skew rider -
compressible skew is the 45.2 us/boundary per-die RATE spread = 3.0-6.1 ms/cycle
(W39), and every kernel win (W37 Opp-1/2/3) compresses it proportionally
(skew = 0.078 x per-boundary work) plus the queued W22-C2 taskset item (0-1% class).
Fabric ceiling bookkeeping (W39 correction): a zero-transport fantasy is bounded by
the WHOLE served ring (128.9 us x 136 = 17.5 ms/cycle), not the 69.5 us emulation
floor (9.5 ms) - and transport is dead anyway.
...
### WALL 3 - Prefill-at-depth collapse (MERGED default-off; U2 served +19%; promotion law-blocked)
...
MERGED + SERVED (E-151..E-153): the branch merged --no-ff as 53d0c97ac, build-hip
rebuilt (binary e247afbbe8167ca3), CI decode-guard PASS arm-off (24.71 vs 24.69).
W40 preflight fixed the U2 harness geometry (512/512 -> 1024/1024). Three U2 SHORT
windows (warm/cool/cold boots) measured **+18.7..+19.8% prefill @10k on ALL SIX pairs,
text sha identical, zero arm failures** (section 5.5) - but promotion is LAW-BLOCKED
by the <=991-duty void gate, which flaps 9-27% at 10k prefill arm-independently
(in-window creep; a cold boot gated WORSE). Arm stays DEFAULT-OFF; owner decision
package formalized (recalibrate the gate for prefill window classes, or promote on the
6-pair evidence + U2 --long curve first).
...
### WALL 5 - The thermal soak (mechanism proven; the floor "cure" killed)
...
soak settles in the U1 window. CANDIDATE CURE KILLED (E-149/W36): the dpm clock floor
does not survive load - f2 leaked into the 991 band under warm soak, the floored 200k
cell latched through the throttle band anyway (VOID, act_mean 1209, duty 22%), and
where the pin held cleanly (f1) decode was SLOWER: the W18 anti-decay claim is refuted
inside the arm built to prove it; any future clock-forcing must come from SMU/od8 level
(not dpm_sclk) and carry a memory-temp guard (HBM 90-93 C vs crit 95 under the 200k
needle). Soak management (idle placement, position-1 law) remains the standing
mitigation. GATE COROLLARY (E-152/E-153): the <=991-duty rule is arm-INDEPENDENT noise
at 10k prefill (9-27% flaps, in-window creep) - decode-class calibration does not
transfer to prefill windows; recalibration sits in the U2 owner decision package.
...
access-pattern ceiling stands as the wall's current face. W37 roofline accounting
(E-150) scoped what is left: 54.7 GB/s effective = 1.3-2.2x off the schedule-family
ceiling (NOT 10x off peak); the win inside the family is bounded -12..-30 ms/cycle;
ranked top-3 = whole-pool schedule/occupancy lift (-3..-14 ms), small-kernel dedup +
TP4 copy deletion (-2.5..-3.5 ms, W41's assignment), attention-tile concurrency
(-2..-4 ms); falsifiable prediction gates written for each (W37 section 5). Getting
past 120 GB/s needs a different schedule (the banked W5 LDS-y staging design) or a
format change (owner-guarded).
...
| 19 | MMVQ S2R gates in serving | NO-PROMOTE (w23env: +0.32% in 0.28 t/s spread; kernels exonerated); gates stay unset | W23-C2, E-145/E-147/W33 |
| 20 | IQ4XS_SHARE re-open | NO-PROMOTE (same window; ~-2.0 ms never materialized served) | W23-C1, E-145/E-147/W33 |
| 21 | per-die clock floors | **KILLED (six-cell root re-run): floor arm lost ALL THREE pairs -0.32/-0.36/-0.14 t/s; dpm high does NOT survive load; decode not sclk-limited in the 1100-1300 band; HBM 90-93 C hazard witness; set_clock_floor.sh stays OUT of the boot chain** | E-149/W36 |
...
| 24 | q4_0-direct PREFILL arm | **MERGED default-off (53d0c97ac, CI PASS arm-off); U2 served: +18.7..+19.8% @10k on ALL SIX pairs across 3 boots, text sha identical, zero arm failures; promotion LAW-BLOCKED by the <=991-duty gate (miscalibrated at 10k prefill); owner decision package** | E-151/E-152/E-153/W27/W40 |
...
| 41 | W37 verify-path roofline accounting | zero-GPU: MMVQ 54.7 GB/s effective = 1.3-2.2x schedule ceiling; strict pool 60.6 ms/cycle (49%); **MODEL CORRECTION: dense qwen35 (16 full-attn + 48 gated-delta-net), NOT MoE**; top-3 kernel opportunities ranked with falsifiable gates; all-kernel stack 26.0-30.0 t/s | E-150/W37 |
| 42 | W38 one-shot allreduce / vLLM custom-AR transport | **DEAD PERMANENTLY: peer access 0/12 pairs (W31) AND hipIpcGetMemHandle invalid-argument 0/12 (IPC probe); even if the substrate existed: +3.2-4.5% max with skew untouched, +7.6% absolute ceiling; payload corrected: all 136 decode boundaries fp32 hidden 5120 (80 KB verify + 20 KB draft); skew = 141 us/boundary of the 210.8 us wall** | E-150/W38 |
| 43 | W39 skew scope (bookkeeping + hypothesis sweep) | compressible skew corrected to **3.0-6.1 ms/cycle** (45.2 us/boundary rate spread; the ~141 us figure is wait redistribution, not wall time); placement imbalance DEAD (every shard divides by 4 exactly); W-coupling law: kernel wins carry a proportional skew rider; no code arm needed | W39 |
| 44 | W40 U2 preflight | READY: blocking 512/512 geometry defect fixed in place (6 hunks); engagement greps verified; oracle carry-over proven (byte-identical prefill region) | W40 |
| 45 | q40-prefill merge + CI | MERGED --no-ff 53d0c97ac; rebuilt binary e247afbbe8167ca3; CI decode-guard PASS arm-off: 24.71 vs 24.69; of-record unchanged | E-151 |
| 46 | git object purge #3 | session bounce (gdm/xrdp restart) left 8 empty objects incl. a mid-write commit + index blob; repaired per playbook: reset to provenance anchor 13ccb8414, purge, index rebuild, fsck clean; zero receipts lost | E-151 |
| 47 | fresh-boot hb geometry battery | **4/4 PASS: ub1024 INDEPENDENTLY CONFIRMED** - 1024/1024 prefill 226.3/226.3, decode 25.06/25.06 (paired +4.9/+5.5% prefill, +2.9/+3.5% decode vs 512); confirmation, not a new ratchet | E-154 |
| 48 | W39 A1/A1b NCCL channel arms | A1 MAX=1 alone never engaged (RCCL clamps at Channel 00/02 -> VOID); A1b MIN=MAX=1 ENGAGED and **LOST -0.20 t/s**; channel axis **BRACKETED CLOSED: 1 worse, 2 optimal, 4 worse (E-119)** | E-155 |
| 49 | W39 A2 NCCL_SHM_USE_CUDA_MEMCPY | **UNUSABLE on RCCL 2.20.5: server init HANGS 2/2 (>800 s in comm init) while controls boot clean; wire term stays SM-store-optimal (W31 3.2 prior)** | E-155 |
...
16. **Git object purge #3 (E-151).** The 20:21-20:22 session bounce (gdm/xrdp restart,
    which also killed U2's first fire mid-cell) left 8 git objects empty, including the
    mid-write rev9 commit 5c0759fe and 1 index blob. Repair per playbook: branch reset
    to the provenance anchor 13ccb8414 (CI had verified that exact hash at 20:19),
    empty objects purged, index rebuilt (rm .git/index + read-tree HEAD), fsck clean;
    zero receipts/ledger lost, all desk branch refs intact; rev9 restated in full at
    c4dcc143f. Third occurrence of the incident-6 class: the snapshot-push-after-every-
    landing law held (mitigation followed), and a CI-verified anchor hash made the
    reset decision mechanical.
17. **The gate that voided a +19% arm (E-152/E-153).** The <=991 MHz-duty void rule -
    calibrated on the E-133 decode VOID class - structurally cannot pass a 4-cell 10k
    prefill window: duty flaps 9-27% arm-INDEPENDENTLY, creeps WITHIN each window
    irrespective of boot state (a cold boot gated WORSE than a warm one's first cells),
    and is not monotone in position, recency, or temperature. Three U2 windows / 12
    cells: zero failures attributable to the arm (no SHA-VOID, no ENG-VOID), two windows
    voided by the gate, promotion blocked by law. Born: per-window-class gate
    calibration (owner decision package); the arm result itself is documented and
    frozen at default-off pending that decision.
...
11. GATE THRESHOLDS ARE WINDOW-CLASS-SCOPED: the <=991-duty rule calibrated on decode
    VOID cells does not transfer to 10k prefill windows (duty flaps 9-27%
    arm-independently with in-window creep, E-152/E-153); a window-class needs its own
    calibration before its void gate is allowed to decide promotions.
12. CLOCK-FORCING HAZARD (recorded, E-149): under the 200k needle HBM runs 90-93 C vs
    crit 95 on both arms; any future clock-forcing attempt must come from SMU/od8 level
    (not dpm_sclk) and carry a memory-temp guard. The dpm clock-floor lever is CLOSED -
    do not re-arm it.
...
2. The arrival skew: 59-141 us/die medians are WAIT REDISTRIBUTION; the honest
   compressible prize is the 45.2 us/boundary rate spread = 3.0-6.1 ms/cycle (+2.4-4.8%,
   W39). The named direct lever - per-die clock floors - was measured under root and
   KILLED (E-149/W36: floors don't survive load; arm lost all pairs; decode is not
   sclk-limited in the 1100-1300 MHz band; HBM hazard). Still open: making the WAIT
   cheap instead of preventing it (overlap the wait with useful per-die work); the
   W-coupling rider (kernel wins compress the skew proportionally); the queued W22-C2
   taskset item (0-1% class).
3. MMVQ: the per-gate/per-type follow-ups are all measured and dead - C4 DEAD
   (oracle-fail iq4_xs, +21.8% iq3_s; FMA-contraction, instantiation-local), occupancy
   rungs DEAD (lb2 no-op; max-vgpr unbuildable on ROCm 6.2 LLVM 18), env window
   NO-PROMOTE (+0.32% in 0.28 spread) (W30/E-144/E-146/E-147). W37 (E-150) re-scoped
   what remains as three ranked whole-pool opportunities with falsifiable gates:
   (1) schedule/occupancy lift into the cfull band (-3..-14 ms -> 25.4-27.4 t/s;
   falsified if served +< 2%); (2) quantize_q8_1 dedup + TP4 copy deletion
   (-2.5..-3.5 ms; W41 live); (3) fa40 tile concurrency (-2..-4 ms). Still open: a
   schedule replacement (W5 LDS-y staging design) that beats the 70-120 GB/s cfull
   ceiling on gfx900 (DP4A, wave64) - the one path past the family wall.
4. Acceptance: head-calibration-bound is now EMPIRICAL - chain-4 drafted 167, accepted
   the identical 84 of chain-3, position-4 acceptance ~0 (E-147). Acceptance itself is
   rock-stable at 0.66667 across every measured cell since (clock-floor, U2, hb
   windows). The W34 fixed-text
   ...
   v2 deep rerun.
5. The ring floor / cheaper exchange: **CLOSED for this stack (W31/E-144, then
   W38/E-150 permanently).** Flat P2P
   ...
   The one open substrate, hipIpcGetMemHandle, was probed and closed: invalid-argument
   0/12 pairs, memcpyPeer 0/12 - "P2P-via-IPC NOT available" (W38 ipc_open log).
   Multicast/shared-memory staging would face the same fabric refusal; the butterfly
   remains 25% slower end-to-end; even a working transport buys only +3.2-4.5% with
   skew untouched.
6. The U2 promotion decision (OWNER, package formalized E-153): the q40-PREFILL arm
   measures +18.7..+19.8% prefill @10k on all six pairs across three boots, text
   sha identical, zero arm failures - but every void came from the decode-calibrated
   <=991-duty gate. Decide: (1) recalibrate the duty rule for prefill window classes
   and re-window, or (2) accept the 6-pair evidence, promote with the prefill ratchet,
   and run the U2 --long 50k/100k curve before production reliance. Until decided, the
   arm stays merged DEFAULT-OFF (zero risk, CI-green).
...
| 09-24 19-21 | transport dead + roofline | W36 clock-floor root retry: **KILL** - floor arm lost all pairs, floor does not survive load, HBM 93 C hazard (E-149); W38 one-shot AR **DEAD permanently** (peer 0/12 + IPC 0/12 invalid-argument), payload corrected to all-fp32/hidden-5120, skew = 141 us of the 210.8 us boundary (E-150); W37 roofline: 54.7 GB/s = 1.3-2.2x schedule ceiling, strict 60.6 ms (49%), **dense-not-MoE model correction**, top-3 kernel opportunities ranked (E-150); q40-prefill MERGED 53d0c97ac + CI PASS; object purge #3 repaired to anchor 13ccb8414; U2 refired (E-151) |
| 09-24 21-09-25 01 | U2 saga + night queue | W40 preflight READY (512->1024 geometry fix); U2 SHORT x2 + a true cold-boot third window: **+18.7..+19.8% prefill on ALL SIX pairs**, text sha identical, zero arm failures - promotion LAW-BLOCKED by the <=991-duty gate (flaps 9-27% arm-independently, in-window creep, cold boot gated worse); owner decision package formalized; arm stays merged default-off (E-152/E-153); fresh-boot hb battery 4/4 PASS - **ub1024 independently confirmed** (1024/1024: prefill 226.3, decode 25.06) (E-154) |
| 09-25 01-04:30 | NCCL arms closed | W39: A1 MAX_NCHANNELS=1 alone never engages (RCCL clamps -> VOID); A1b MIN=MAX=1 ENGAGED and LOST -0.20 t/s -> **channel axis BRACKETED CLOSED: 1 worse, 2 (default) optimal, 4 worse**; A2 NCCL_SHM_USE_CUDA_MEMCPY HANGS server init 2/2 -> unusable on RCCL 2.20.5; the allreduce wall's env-knob tier exhausted; remaining lever = skew rider on kernel wins (E-155) |

*End of dossier. When this document and the ledger disagree, the ledger wins
(append-only, authoritative). Latest ledger head at writing: E-155, commit 53a2b70cd.*
Queued behind this sync: the U2 owner decision (recalibrated-gate re-window vs promote
on the 6-pair evidence; U2 --long 50k/100k curve if promoted), W41 smallk desk
(Opp-2 quantize_q8_1 dedup + TP4 copy deletion, wt-smallk), W37 Opp-1 whole-pool MMVQ
schedule desk (fleet resets 11:25), U1 v2 deep rerun (deferred until the lever queue
drains), W22-C2 taskset (0-1% class).
