// Chunked-dequant desk: census-shape bench of the tile route unchunked
// (GGML_CUDA_TILE_FP16=1, per-call full-weight f16 dequant - the alloc class that
// OOMs at 200k, E-044) vs chunked (GGML_CUDA_TILE_FP16=1
// GGML_CUDA_TILE_FP16_CHUNKED=1, per-slice dequant into a small reusable window)
// through the real ggml mul_mat dispatch, single device. M=128 census prefill
// shapes with per-die N shares and layer call counts (E-012 census / E-015
// weights), IQ3_S weights. Wall op time via host timers (dequant + src1 convert +
// GEMM in both arms). GEMM-only kernel time comes from a rocprofv3 kernel trace of
// this same binary. --dump writes the f32 dst per shape for the byte-identity
// oracle: chunked-vs-unchunked dst must be bit-identical (same slice partition,
// same half2 schedule, same f32 slice order; window dequant runs the same
// to_fp16 kernels on the same quant blocks).
//
...
// (pool blocks + slice window), against the 200 MiB free envelope at TP3/200k.
//
// Build: g++ -O2 -std=c++17 -I ggml/include bench_tile_chunked.cpp \
//   -L build-tile-chunked/bin -lggml -lggml-base -lggml-hip -Wl,-rpath,<abs bin>
// Run (die 3): HIP_VISIBLE_DEVICES=3 /tmp/bench_tile_chunked [m] [reps] [iters] [--dump]
...
int main(int argc, char ** argv) {
    ...
    const char * env_chunked = getenv("GGML_CUDA_TILE_FP16_CHUNKED");
    const char * env_win     = getenv("GGML_CUDA_TILE_FP16_CHUNKED_MIB");
    const char * arm = env_chunked && env_chunked[0] == '1' ? "CHUNKED" : "UNCHUNK";
    printf("tile-chunked bench [%s]: M=%lld reps=%d x %d iters | TILE_FP16=%s CHUNKED=%s WIN_MIB=%s | free %.1f MiB\n",
           arm, (long long) m, reps, iters,
           env_tile ? env_tile : "unset", env_chunked ? env_chunked : "unset",
           env_win ? env_win : "default", base_free / 1048576.0);

    // free-VRAM sampler: catches every transient (pool blocks + slice window);
    ...
    for (int si = 0; si < NSHAPES; si++) {
        ...
        // warmup: route selection, slice-window alloc, lmax growth
        for (int i = 0; i < 3; i++) {
        ...
        if (dump) {
            ...
            char path[256];
            snprintf(path, sizeof(path), "/tmp/tile_chunked_dump_s%d_m%lld_%s.bin", si, (long long) m, arm);
