...
# provenance law (E-112): attribute the served binary + launch recipe
PROV_BIN="$HERE/../../../build-hip/bin/llama-server"
if [ -x "$PROV_BIN" ]; then
  BATTERY_CMD+=(--server-binary "$PROV_BIN"
                --launch-config "BOOT_SCRIPT=${BOOT_SCRIPT:-unset} port=$PORT")
fi
