...
enum ArmKind { ARM_BASE, ARM_MID3, ARM_WIDE6, ARM_DIRECT,
               ...
               ARM_FILL56, ARM_FILL84, ARM_FILLMAX, ARM_FILLDUP, ARM_V11DUP };
...
static const char * arm_name(ArmKind k) {
    switch (k) {
        ...
        case ARM_FILL56:  return "fill56";  // W43: v11 + KV-split pb=56
        case ARM_FILL84:  return "fill84";  // W43: v11 + KV-split pb=84
        case ARM_FILLMAX: return "fillmax"; // W43: v11 + max split (pb=ntiles_KV)
        case ARM_FILLDUP: return "filldup"; // W43: fill84 again (determinism control)
        case ARM_V11DUP:  return "v11dup";  // W43: v11 again (in-run control)
    }
    return "?";
}
...
// W43 attn-concurrency arms: the promoted v11 instance unchanged, only the
// KV-split height (parallel_blocks) pinned via the launch_fattn fill param.
// FILL > 0 -> explicit value (clamped to ntiles_KV); FILL < 0 -> max split.
// Kernel body and store shapes untouched; the combine sum order changes with
// the value, so fill arms are DUST-class vs v11 by design and the determinism
// control (filldup vs fill84) must stay BIT-EXACT.
template <int VAR, int FILL>
static void launch_arm_q40_fill(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    const int nthreads  = ggml_cuda_fattn_tile_get_nthreads (DKQ, DV, 8, cc);
    const int nbatch_fa = ggml_cuda_fattn_tile_get_nbatch_fa(DKQ, DV, 8, cc);
    launch_fattn<DV, 4, 2>(ctx, dst,
        flash_attn_tile<DKQ, DV, 4, 2, false, VAR>,
        nthreads / 32, 0, nbatch_fa, false, false, false, 32, FILL);
    HIP_CHECK(hipGetLastError());
}
...
static void run_arm(ArmKind k, ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    switch (k) {
        ...
        case ARM_FILL56:  launch_arm_q40_fill<11, 56>(ctx, dst); break;
        case ARM_FILL84:  launch_arm_q40_fill<11, 84>(ctx, dst); break;
        case ARM_FILLMAX: launch_arm_q40_fill<11, -1>(ctx, dst); break;
        case ARM_FILLDUP: launch_arm_q40_fill<11, 84>(ctx, dst); break;
        case ARM_V11DUP:  launch_arm_q40<11>(ctx, dst); break;
    }
}
...
int main(int argc, char ** argv) {
    if (argc < 2) {
        printf("usage: %s <gguf> [niter=30] [reps=5] [--served-entry] [--occupancy] [--probe] [--matrix] [--depth N] [--M N] [--used N] [--prefill] [--conc] [--allarms]\n", argv[0]);
        return 1;
    }
    ...
    bool conc        = false; // W43 concurrency arm set: v11/v11dup/fill56/fill84/fillmax/filldup
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (strcmp(argv[i], "--conc") == 0)         conc = true;
    ...
    const std::vector<ArmKind> arms = prefill ? std::vector<ArmKind>{ ARM_BASEP, ARM_V11P, ARM_BASEDUPP }
                                      : conc   ? std::vector<ArmKind>{ ARM_V11, ARM_V11DUP, ARM_FILL56, ARM_FILL84, ARM_FILLMAX, ARM_FILLDUP }
    ...
    if (conc) {
        // pin the v11 instance census into every W43 log (W14 attrs class)
        hipFuncAttributes attr{};
        HIP_CHECK(hipFuncGetAttributes(&attr, (const void *) &flash_attn_tile<DKQ, DV, 4, 2, false, 11>));
        printf("CENSUS v11: numRegs=%d static_shared=%zu local=%zu\n",
               attr.numRegs, attr.sharedSizeBytes, attr.localSizeBytes);
    }
    ...
    for (size_t a = 0; a < arms.size(); ++a) {
        ...
        if (a > 0 && (arms[a] != ARM_BASEDUP) && (arms[a] != ARM_FILLDUP) && (arms[a] != ARM_V11DUP)) {
        ...
        if (arms[a] == ARM_V11DUP) {
            if (out[a] != out[0]) {
                printf("ORACLE FAIL: v11dup != v11 (nondeterministic v11 control)\n");
                return 1;
            }
            printf("ORACLE %s vs v11: BIT-EXACT (in-run control)\n", arm_name(arms[a]));
        }
        if (arms[a] == ARM_FILLDUP) {
            const size_t fi = (size_t) (std::find(arms.begin(), arms.end(), ARM_FILL84) - arms.begin());
            if (out[a] != out[fi]) {
                printf("ORACLE FAIL: filldup != fill84 (nondeterministic fill arm)\n");
                return 1;
            }
            printf("ORACLE %s vs fill84: BIT-EXACT (determinism control)\n", arm_name(arms[a]));
        }
    }
    for (size_t a = 1; a < arms.size(); ++a) {
        if (arms[a] == ARM_BASEDUP || arms[a] == ARM_FILLDUP || arms[a] == ARM_V11DUP) continue;
