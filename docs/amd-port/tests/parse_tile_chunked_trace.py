#!/usr/bin/env python3
# Parse the two rocprofv3 --kernel-trace CSVs of bench_tile_chunked.cpp runs
# (unchunked and chunked arms) into per-shape kernel-only medians, us per
...
# attributed in time order by the launch model:
#   unchunk: per compute 1 gemm + 1 reduce + 1 weight dequant + 1 src1 convert
#   chunked: per compute ks*(gemm + gather + dequant) + 1 src1 convert
...
for arm, path in (("UNCHUNK", sys.argv[1] + "unchunk_kernel_trace.csv"),
                  ("CHUNKED", sys.argv[1] + "chunked_kernel_trace.csv")):
    rows = load(path)
    ksg = [s[3] for s in SHAPES]
    if arm == "UNCHUNK":
        base = [1] * len(SHAPES)                 # slices per compute
        streams = {
            "gemm":   ([(n, d) for n, d in rows if "tile_fp16_gemm" in n], 1),
            "route":  ([(n, d) for n, d in rows if "tile_fp16_gemm" in n or "tile_fp16_reduce" in n], 2),
        ...
        streams = {
            "gemm":   ([(n, d) for n, d in rows if "tile_fp16_gemm" in n], 1),
            "route":  ([(n, d) for n, d in rows if "tile_fp16_gemm" in n or "tile_chunk_gather_q" in n or "dequantize" in n], 3),
            "dequant":([(n, d) for n, d in rows if "dequantize" in n or "tile_chunk_gather_q" in n], 2),
        }
    print(f"== {arm} kernel-only (median us/compute over last {reps*iters} computes)")
    res = {}
    for label, (stream, mult) in streams.items():
        res[label] = per_compute(stream, [mult * b for b in base])
