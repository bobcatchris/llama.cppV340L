...
//   lb4s      share body forced to the LB4 64-VGPR budget (W30 occ-dumped,
//             never timed; the served dispatcher expresses it as
//             GGML_CUDA_MMVQ_LB=4 + the *_SHARE gates)
//   rpb1      W42 register diet: one-row share blocks (rows_per_cuda_block
//             1, grid (N,1,1)) - halves the dec[2][..] state so the T=2-4
//             share arms fit 8 CTAs/CU without spill
//   rpb1lb4   rpb1 + the LB4 launch_bounds hint (forces the 8-CTA budget)
...
struct ArmSpec {
    std::string name;   // base | share | s2r | lb2 | lb4 | lb4s | rpb1 | rpb1lb4 | pair | fused
};

enum ArmKind { ARM_BASE, ARM_SHARE, ARM_S2R, ARM_LB2, ARM_LB4, ARM_LB4S, ARM_RPB1, ARM_RPB1LB4, ARM_PAIR, ARM_FUSED };
...
static ArmKind arm_kind(const ArmSpec & a) {
    ...
    if (a.name == "lb4s")  return ARM_LB4S;
    if (a.name == "rpb1")   return ARM_RPB1;
    if (a.name == "rpb1lb4") return ARM_RPB1LB4;
...
// RPB1 arms launch the W42 one-row variant with grid (N, 1, 1))
template <ggml_type type, int T, bool S2R, int LB = 1, bool RPB1 = false>
static void launch_k(const void * vx, const block_q8_1 * vy, float * dst,
    ...
    mul_mat_vec_q<type, T, false, false, false, S2R, LB, RPB1>
        <<<grid, block, 0, stream>>>(
...
static void occ_k(const char * name) {
    ...
    const void * kp = (const void *) &mul_mat_vec_q<type, T, false, false, false, S2R, LB, RPB1>;
...
static void dispatch_arm(const ArmSpec & a, const void * vx, const void * vgate, const block_q8_1 * vy,
                         ...
                         const uint32_t stride_col_dst, const uint32_t stride_channel_y,
                         const uint32_t stride_sample_y, const uint32_t stride_channel_dst,
                         const uint32_t stride_sample_dst, dim3 grid, dim3 grid1, dim3 block,
...
#define LKR(S2R_, SHARE_, LB_, RPB1_) launch_k<type, T, S2R_, LB_, RPB1_>(vx, vy, dst, K, \
        stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, \
        stride_channel_dst, stride_sample_dst, RPB1_ ? grid1 : grid, block, stream, SHARE_)
...
#define OKR(S2R_, LB_, RPB1_) occ_k<type, T, S2R_, LB_, RPB1_>(nm)
    switch (k) {
        case ARM_BASE:
        ...
        case ARM_LB4:
            ...
            break;
        case ARM_LB4S:
            // the served LB4 composition the dispatcher never times: share
            // body forced to the 64-VGPR budget (W30 occ-dumped, not timed)
            if (occupancy) { OKR(false, 4, false); return; }
            LKR(false, true, 4, false);
            break;
        case ARM_RPB1:
            // W42 register diet: one-row share blocks, grid (N,1,1)
            if (occupancy) { OKR(false, 1, true); return; }
            LKR(false, true, 1, true);
            break;
        case ARM_RPB1LB4:
            // diet + launch_bounds hint: forces the 8-CTA budget
            if (occupancy) { OKR(false, 4, true); return; }
            LKR(false, true, 4, true);
            break;
        case ARM_PAIR:
            if (occupancy) { OK(false, 1); return; }
...
static void dispatch_T(int T, const ArmSpec & a, const void * vx, const void * vgate, const block_q8_1 * vy,
    ...
    if (T == 2) {
        dispatch_arm<type, 2>(a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y,
            ...
            stride_sample_dst, grid, grid1, block, stream, occupancy);
    } else if (T == 4) {
...
static void dispatch_type(ggml_type type, int T, const ArmSpec & a, const void * vx, const void * vgate,
                          ...
                          dim3 grid, dim3 grid1, dim3 block, hipStream_t stream, bool occupancy) {
    switch (type) {
        case GGML_TYPE_IQ3_XXS: dispatch_T<GGML_TYPE_IQ3_XXS>(T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, stride_channel_dst, stride_sample_dst, grid, grid1, block, stream, occupancy); break;
        case GGML_TYPE_Q4_K:    dispatch_T<GGML_TYPE_Q4_K>   (T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, stride_channel_dst, stride_sample_dst, grid, grid1, block, stream, occupancy); break;
        case GGML_TYPE_IQ4_XS:  dispatch_T<GGML_TYPE_IQ4_XS> (T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, stride_channel_dst, stride_sample_dst, grid, grid1, block, stream, occupancy); break;
        case GGML_TYPE_Q5_K:    dispatch_T<GGML_TYPE_Q5_K>   (T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, stride_channel_dst, stride_sample_dst, grid, grid1, block, stream, occupancy); break;
        case GGML_TYPE_Q6_K:    dispatch_T<GGML_TYPE_Q6_K>   (T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, stride_channel_dst, stride_sample_dst, grid, grid1, block, stream, occupancy); break;
        case GGML_TYPE_Q3_K:    dispatch_T<GGML_TYPE_Q3_K>   (T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, stride_channel_dst, stride_sample_dst, grid, grid1, block, stream, occupancy); break;
        case GGML_TYPE_IQ3_S:   dispatch_T<GGML_TYPE_IQ3_S>  (T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y, stride_col_dst, stride_channel_y, stride_sample_y, stride_channel_dst, stride_sample_dst, grid, grid1, block, stream, occupancy); break;
...
int main(int argc, char ** argv) {
    ...
    std::vector<ArmSpec> arms;
    {
        ...
        for (char * tok = strtok(dup, ","); tok; tok = strtok(nullptr, ",")) {
            ...
            const char * legal[] = { "base", "share", "s2r", "lb2", "lb4", "lb4s", "rpb1", "rpb1lb4", "pair", "fused" };
            bool ok = false;
    ...
    for (const auto & a : arms) {
        ...
        if (fusion_mode != fusion_arm) {
            printf("a C4 fusion run takes only pair/fused arms; a base run takes only base/share/s2r/lb2/lb4/lb4s/rpb1/rpb1lb4\n");
            return 1;
        }
    ...
    dim3 grid1((unsigned) N, 1, 1);   // W42 rpb1 arms: one row per CTA
    dim3 block(64, 2, 1);
    ...
    if (occupancy) {
        for (const auto & a : arms) {
            dispatch_type(ti->type, T, a, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y,
                          ...
                          (uint32_t) N*T, (uint32_t) N*T, grid, grid1, block, stream, true);
        }
    ...
    std::vector<float> arm_out((size_t) N*T);
    {
        ...
        dispatch_type(ti->type, T, a0, vx, vgate, vy, dst, dst_b, K, stride_row_x, stride_col_y,
                      N, (uint32_t) T*stride_col_y, (uint32_t) T*stride_col_y,
                      (uint32_t) N*T, (uint32_t) N*T, grid, grid1, block, stream, false);
