...
## REVISION 16 - 2026-09-25 07:30 (death #6 handled; persist-2 protocol; both desks re-dispatched; owner decisions 2+3 in motion)

- DEATH #6 at 06:59:45 (mid persistent-c3): machine rebooted 06:59:50,
  recovery CLEAN (fsck clean, rev15 intact, boot queue self-fired - hb
  cells running until ~07:30). Death #6 is the FIRST under the persistent
  protocol: it tracks in-server KV/allocation churn too, not just boots.
  RULE: protocol persist-2 = max 2 cells per server boot. Baseline partial:
  c1 24.88 / c2 24.95 (both PASS, protocol persist-1, dead boot); c3/c4
  re-run as a fresh 2-cell persist-2 run after the hb queue drains.
- OWNER DECISIONS RECEIVED: (2) persistent-server = YES (implemented,
  baseline in progress); (3) L7 = per-layer recipe mandate (desk v2 live);
  (1) prefill arm = owner asked for downsides, answer delivered (depth
  invariance at 50k+ is the only unverified axis), awaiting go/park.
- DESKS: Opp-3 attn v2 (wt-attn3, resumed) + L7 recipe v2 (main docs) live.
- NEXT: hb queue drains -> persist-2 control (2 cells) -> bank E-159 with
  the full persistent baseline -> Opp-3 bench slot when its arm lands ->
  L7 quality-gate windows when its recipe lands.

## REVISION 17 - 2026-09-25 09:05 (death #7; churn throttle; L7 gate in flight; arxiv 2609.26333 reviewed)

- DEATH #7 at ~08:47 (boot aged 1h47m): second hard death in 2 h. The
  06:59-08:47 window ran ~14 server boot/teardown cycles (hb queue + two
  failed-fast ABBA attempts) = 2.3x the proven-safe boot rate (yesterday's
  clean battery: 1 boot/19 min). ROOT RESPONSE: hog_gate added to the ABBA
  wrapper (pause 300 s whenever the svm hog counter >= 2; deaths at 4-5),
  refired as campaign-l7-abba4 on the fresh 08:48 boot. fsck CLEAN both
  deaths (#6 and #7) - no purges.
- HARNESS FIXES that un-blocked the L7 gate (three silent-fault layers):
  (1) run_tp3_guards.sh now passes --server-binary/--launch-config (E-112
  law postdates the desk3 wrapper); (2) PROV_BIN path depth fixed
  (tests/../.. lands in docs/, needs three ups); (3) the ABBA manifest
  untracked (per-boot appends dirtied the clean-tree gate). Also verified
  live: the Tier A gguf BOOTS AND SERVES (Ready ~25 s, gate boot c1_B).
- ARXIV 2609.26333 (Disaggregated Quantization, NVIDIA/ISTA) reviewed for
  applicability - verdict: validates the campaign's phase-split serving
  (paper's core thesis = our fa40/U2 arm split); sharpens the L7 gate
  (decode-side quant quality loss dominates - the output-head move P1 is
  the watch item; drop P1 first if acceptance dips); LUT2/LUT3-class
  formats noted as the post-W37-redesign L7 direction (byte savings only
  pay once the schedule ceiling lifts). No wholesale adoption (NVFP4/QADD
  need NVIDIA tensor cores + training; ODP does not fit our VRAM bind).
  Receipt: W45 (next cycle).

## REVISION 18 - 2026-09-25 10:45 (STORM: GPU churn stopped; owner hardware attention requested; state of record)

- DEATHS #6/#7/#8 within 3h53m (06:59, 08:47, 09:52) - the box now dies
  under any sustained GPU churn. GPU WORK STOPPED by coordinator decision
  (E-160). Dies idle 36C, zero servers. The prefill-arm 200k invariance
  cell, Tier A-cons gate, W43 bench, and persist-2 control are ALL QUEUED
  behind a calm-machine period / owner hardware inspection.
- OWNER HARD WARNING: three deaths in an afternoon is a hardware
  trajectory (power delivery or HBM degradation under burst; NOT
  temperature-correlated - deaths start from 27-36C cool dies). Yesterday
  the same work profile ran 25+ boots clean. Inspect before resuming.
- COMPLETED THIS ERA (all banked, pushed through E-160 / d4e4c413b):
  prefill arm PROMOTED per owner go (BASEENV flipped everywhere; 200k
  invariance cell pending calm machine); L7 Tier A gate RAN and FAILED
  clean (needle 1/3, 2/3 + first-ever in-boot nondeterminism; baseline
  2/2 PASS) -> demoted to Tier A-cons (artifact built, census-verified,
  sha16 18da578b, head preserved); W43 Opp-3 diagnosis + falsifier arm
  banked (amd/attn3); quantize tool copy-guard fix; run_tp3_guards
  provenance fixes; ABBA receipts banked. Ladder lesson: --tensor-type-file
  patterns are FIRST-MATCH REGEX - anchor and escape everything, census-
  verify every artifact.
- WHEN THE MACHINE IS CLEARED: rebuild llama-server (src fix landed) ->
  prefill invariance cell -> Tier A-cons gate (2 boots vs today's A pair)
  -> W43 bench -> persist-2 control. In that order, one item per boot.

## REVISION 19 - 2026-09-25 14:55 (of-record ratcheted to 25.33/244.18; cheap-lever era COMPLETE; agenda is owner-gated)

- OF-RECORD (E-166): decode 25.33 t/s (was 24.69), prefill 244.18 t/s (was
  217.71), accept 0.66-0.69, geometry 1024/1024, prefill arm ON in BASEENV.
  The ratchet is conservative (lowest of three post-promotion measurements:
  25.42 invariance / 25.33 + 25.32 persist).
- THE CHEAP-LEVER ERA IS COMPLETE. Closed by measurement this era: clock
  floors (E-149), one-shot AR/IPC transport (W38/E-150), NCCL knobs
  (E-155), W37 Opp-1/2/3 (E-156/E-157/E-165), L7 iq4_xs family (E-164).
  Surviving measured assets: promoted prefill arm (+12.2 pct prefill),
  ub1024 geometry, fa40 decode arm, all share/env arms in BASEENV.
- REMAINING AGENDA (owner-gated): U2 --long depth curve (~7 h run);
  U1 v2 deep rerun; L7 via true low-bit formats (needs f16 source ~55 GB
  + imatrix regen); ROCm 6.3+/kernel upgrade + memtest/PSU check for the
  death class (E-162); C2 taskset (queued, W22 kill law).
- GPU QUEUE: EMPTY. Machine healthy on the throttled regime (watchdog v2
  active, 5 h+ uptime through churn, 2 live saves). Resume law: one item
  per boot, hog-gated, >= 10 min spacing.
