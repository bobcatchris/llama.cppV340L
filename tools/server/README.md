...
| `--kv-admission, --no-kv-admission` | with unified KV, check the global cache occupancy before launching a task and defer it if the cache cannot supply its prompt (default: enabled)<br/>(env: LLAMA_ARG_KV_ADMISSION) |
| `--kv-fifo-fill, --no-kv-fifo-fill` | fill slots in FIFO order (oldest task first), so a newer request cannot starve an older mid-prompt one (default: enabled)<br/>(env: LLAMA_ARG_KV_FIFO_FILL) |
